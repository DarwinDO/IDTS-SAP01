...
- **Mandatory IDTS SAP490 XLSX skill:** every task that creates, edits, reviews, renders, validates, regenerates, or synchronizes an IDTS SAP490 `.xlsx` workbook or its generator must invoke `.agents/skills/idts-sap490-xlsx-fidelity`. The matching workbook in Google Drive `OFFICIAL SUBMISSION` is the formatting authority. Use candidate-only writes, tab/range approval, structural/style validation, and Excel plus PDF cross-render review before replacing a current artifact or updating Drive.
