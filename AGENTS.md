...
Record the issue immediately after it is observed and before moving to an unrelated implementation step. Do not wait until the end of the session, PR, or weekly consolidation. If the root cause is not known yet, create a provisional status entry with the observed symptom and mark the cause/status as under investigation; update that same entry after diagnosis. Classify each issue as a product defect, environment blocker, tooling issue, test-harness issue, data issue, documentation issue, or process issue so environment/tooling failures are not mislabeled as product bugs.

Member status files are also the temporary collection source for later SAP490 Test And Fix Bug and related deliverables. Record all discovered issues in the session log. Add meaningful SAP490 candidates to the member's temporary bug staging register when one exists, including fixed-in-session issues. DonHV later decides which staged entries belong in the formal SAP490 artifacts; agents must not silently omit an issue merely because it was fixed quickly.
...
Phải ghi nhận lỗi ngay sau khi quan sát thấy và trước khi chuyển sang một bước implementation không liên quan. Không chờ đến cuối phiên, cuối PR hoặc buổi tổng hợp tuần. Nếu chưa biết root cause, tạo ngay một entry tạm với triệu chứng đã thấy và đánh dấu nguyên nhân/trạng thái là đang điều tra; sau khi chẩn đoán thì cập nhật lại chính entry đó. Mỗi issue phải được phân loại rõ là product defect, environment blocker, tooling issue, test-harness issue, data issue, documentation issue hoặc process issue để không ghi nhầm lỗi môi trường/tool thành bug sản phẩm.
