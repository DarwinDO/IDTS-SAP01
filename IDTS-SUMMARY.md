...
**Controlled legacy-link decision:** PM+UserAdmin may send a one-time identity-link invitation for one selected active legacy TESTER or DEVELOPER. After validated SAP login and read-only proof of the exact existing IDTS Role Collection, CAP updates the same `Users.ID` with the verified contact email and immutable identity tuple. It never creates a replacement user, guesses a match, or moves existing Developer Profiles, responsibilities, Bugs, comments, notifications, or history. Until this link is complete, a legacy Developer is excluded from new direct assignment and Smart Assign while existing Bug assignees remain unchanged.

**Vietnamese:** PM+UserAdmin co the gui invitation link identity mot lan cho mot legacy TESTER hoac DEVELOPER da chon. Sau khi SAP login duoc validate va broker chi-doc chung minh dung Role Collection IDTS hien co, CAP cap nhat email lien he va immutable identity tren chinh `Users.ID` cu. He thong khong tao user thay the, khong doan mapping, khong chuyen Developer Profile, responsibility, Bug, comment, notification hoac history. Truoc khi link hoan tat, legacy Developer bi loai khoi assignment moi va Smart Assign, con Bug da assign van giu nguyen.
...
**Access lifecycle extension:** PM+UserAdmin can suspend a provisioned user as an IDTS-local emergency/temporary lock. Suspension protects the final active PM + UserAdmin, revokes active sessions atomically, records `SUSPENDED` and audit, and performs no external provider write. Reactivation queues a read-only `REACTIVATE` operation and leaves local access blocked until the broker proves the immutable principal and exact desired Role Collection set; only then may CAP set the request and internal user to `ACTIVE`. Active Users actions are state-bound, reason/confirmation gated, and show queued outcomes rather than claiming provider success.

**Vietnamese:** PM+UserAdmin co the suspend user da provision nhu mot khoa tam thoi/emergency local cua IDTS. Suspension bao ve PM + UserAdmin active cuoi cung, revoke session trong cung transaction, ghi `SUSPENDED` va audit, khong write ra provider. Reactivation queue operation `REACTIVATE` chi-doc va giu local access bi khoa cho den khi broker chung minh immutable principal va exact Role Collection mong muon; sau do CAP moi set request va user thanh `ACTIVE`. Action tren Active Users bam theo state, bat buoc reason/confirmation va hien ket qua queued thay vi tu nhan provider thanh cong.
...
### **5.8.3. My Notifications and Prompt Delivery Direction**

**English:** The approved next notification architecture adds one personal federated inbox index for Bug and material User Access events while keeping domain source/audit and delivery records authoritative. Important Bug status/ownership events, `@mention`, Critical/Blocker escalation, role change, and successful reactivation use the existing post-commit immediate worker kick and must not wait for hourly recovery. Job Scheduler remains responsible for retry/recovery, Pending Assignment/Overdue discovery, four-hour Critical/Blocker or 24-hour normal SLA reminders, and weekday 08:00 `Asia/Bangkok` digest generation. N5-Lite adds only safe read-only Digest diagnostics to the existing PM/UserAdmin Operations table; manual Digest retry and automated inbox cleanup are deferred until measured need. PM/UserAdmin Operations diagnostics do not grant access to another user's inbox. The first release excludes WebSocket/push, user channel preferences, Work Zone integration, new brokers, workers, or providers.

**Vietnamese:** Kiến trúc notification tiếp theo đã duyệt thêm một inbox index cá nhân liên nguồn cho event Bug và User Access quan trọng, đồng thời giữ source/audit và delivery record theo domain làm authority. Event status/ownership Bug quan trọng, `@mention`, escalation Critical/Blocker, role change và reactivation thành công dùng immediate worker kick sau commit hiện có và không được chờ recovery một giờ. Job Scheduler tiếp tục chịu trách nhiệm retry/recovery, phát hiện Pending Assignment/Overdue, nhắc SLA bốn giờ cho Critical/Blocker hoặc 24 giờ cho mức thường và tạo digest 08:00 ngày thường theo `Asia/Bangkok`. N5-Lite chỉ thêm diagnostic Digest read-only an toàn vào bảng Operations PM/UserAdmin hiện có; retry Digest thủ công và cleanup inbox tự động được defer đến khi có nhu cầu đo được. Operations diagnostic không cấp quyền đọc inbox user khác. Release đầu loại WebSocket/push, channel preference theo user, Work Zone integration, broker, worker hoặc provider mới.
...
## Business Catalog Administration

**English.** An active PM with User Administration capability may create, edit, activate, and safely deactivate IDTS SAP Modules, Application Components, Defect Categories, and valid Component Category pairs. Catalog codes/pairs are unique, Component Category parents must be active, and deactivation is blocked while active responsibilities or catalog children depend on the item. Historical Bugs remain readable. No catalog row is hard-deleted.

**Tiếng Việt.** PM active có User Administration capability được create, edit, activate và deactivate an toàn SAP Module, Application Component, Defect Category và cặp Component Category hợp lệ của IDTS. Code/cặp catalog phải unique, parent của Component Category phải active, và deactivate bị chặn khi còn responsibility hoặc child catalog active phụ thuộc. Bug lịch sử vẫn đọc được. Không hard-delete catalog row.

## Operations and Audit usability baseline

**English:** PM + UserAdmin receive separate bounded Operations and Audit views over onboarding deliveries, access operations, and append-only administration audit. CAP returns explicit safe DTOs with stable ordering, default page size 25 and maximum 100, masked recipient/actor/target display, allowlisted summaries, and a 12-character non-reversible correlation fingerprint. Readiness is derived only from persisted safe outcomes modified within a fixed seven-day freshness window; it does not inspect endpoints, bindings, credentials, provider state, or logs. Delivery retry accepts only an exact failed transient row whose parent invitation remains `INVITED` and unexpired, with optimistic `modifiedAt`, a fixed attempt ceiling, atomic reset plus audit, and the existing post-commit outbox kick. Ambiguous access outcomes expose Reconcile only; permanent failures expose neither Retry nor Reconcile. The UI has Delivery and Provisioning subtabs, a separate Audit tab, lazy models, safe details, and accessible loading/empty/error states. Infrastructure controls, raw email/provider content, lock/lease/idempotency values, identity hashes, tokens, and raw errors remain out of scope.

**Tiếng Việt:** PM + UserAdmin có các view Operations và Audit riêng, có giới hạn, cho onboarding delivery, access operation và audit administration append-only. CAP trả safe DTO explicit với order ổn định, page mặc định 25 và tối đa 100, mask recipient/actor/target, summary allowlist và correlation fingerprint 12 ký tự không đảo ngược. Readiness chỉ derive từ safe outcome đã persist với `modifiedAt` trong freshness window cố định bảy ngày; không inspect endpoint, binding, credential, provider state hoặc log. Retry delivery chỉ nhận đúng row FAILED transient có parent invitation còn `INVITED` và chưa hết hạn, với optimistic `modifiedAt`, attempt ceiling cố định, reset kèm audit atomic và existing outbox kick sau commit. Access outcome ambiguous chỉ hiện Reconcile; permanent failure không hiện Retry hay Reconcile. UI có subtab Delivery/Provisioning, tab Audit riêng, model lazy, detail an toàn và trạng thái loading/empty/error accessible. Control hạ tầng, raw email/provider content, lock/lease/idempotency, identity hash, token và raw error nằm ngoài scope.
