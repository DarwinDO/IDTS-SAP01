...
**Source-candidate extension:** After identity verification, a selected PM with `UserAdmin` must explicitly approve the versioned request. CAP creates a durable access-operation journal and append-only safe audit event; a separate broker is the only component permitted to reconcile allowlisted BTP Role Collections. `ACTIVE` is committed only after broker readback proves the exact desired state. Role change and revoke first disable the local user and revoke active sessions. A retry is allowed only for an explicitly retryable failure; an ambiguous provider outcome requires a separate human-confirmed reconciliation action that reads provider state before applying any missing delta. The broker and its privileged credential are excluded from the ordinary application MTA and have a separate deployment lifecycle. The real SAP API adapter/credential and HANA/XSUAA rollout are not enabled by this source change and require a separate exact approval gate.
