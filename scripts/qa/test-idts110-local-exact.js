...
async function main () {
  ...
  const uiCsn = await cds.load('*')
  cds.compile.to.edmx(uiCsn, { service: 'BugService' })
  ...
  const attachmentFacets = (uiCsn.definitions['BugService.Bugs']['@UI.Facets'] || [])
    .filter(facet => facet && facet.Target === 'attachments/@UI.LineItem')
  await runStaticBlockedCase('UT-ATT-007', ['latest dev uses the SAP-standard generated attachment facet', 'retired custom MIME selection handler is absent', 'replacement AcceptableMediaTypes contract is declared'], async () => {
    ...
    assert.doesNotMatch(attachmentAnnotations, /attachments\/@UI\.LineItem/)
    assert.equal(attachmentFacets.length, 1)
    assert.equal(attachmentFacets[0].ID, 'attachments_attachments')
