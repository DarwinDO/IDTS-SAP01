...
async function main () {
  ...
  assert.equal(manifest['sap.app'].applicationVersion.version, '0.0.8')
  assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'app/bug-management-ui/package.json'))).version, '0.0.8')
  const appLock = JSON.parse(fs.readFileSync(path.join(root, 'app/bug-management-ui/package-lock.json')))
  assert.equal(appLock.version, '0.0.8')
  assert.equal(appLock.packages[''].version, '0.0.8')
