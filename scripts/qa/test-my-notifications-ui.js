...
async function main () {
  ...
  assert.match(index, /body#content\s*\{[\s\S]*display:\s*flex;[\s\S]*flex-direction:\s*column;[\s\S]*overflow:\s*hidden;/, 'protected shell must stack its notification toolbar and Fiori viewport')
  assert.match(index, /body#content\s*>\s*\[data-name=["']idts\.bugmanagementui["']\]\[data-id=["']container["']\]\s*\{[\s\S]*flex:\s*1 1 auto;[\s\S]*min-height:\s*0;/, 'only the exact UI5 component wrapper may consume the viewport remaining below the notification toolbar')
  assert.doesNotMatch(index, /body#content\s*>\s*div:not\(/, 'UI5 static areas and browser-extension-injected sibling divs must not become flex-growing application wrappers')
  assert.match(index, /#container,\s*#container-uiarea\s*\{[\s\S]*height:\s*100%;/, 'UI5 component container descendants must fill the bounded wrapper')
  assert.equal(manifest['sap.app'].applicationVersion.version, '0.0.14')
  assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'app/bug-management-ui/package.json'))).version, '0.0.14')
  const appLock = JSON.parse(fs.readFileSync(path.join(root, 'app/bug-management-ui/package-lock.json')))
  assert.equal(appLock.version, '0.0.14')
  assert.equal(appLock.packages[''].version, '0.0.14')
  assert.doesNotMatch(index, /body\s*>\s*div:not\([^}]+height:\s*100%/, 'shell children must not each consume a full viewport')
