...
async function main () {
  ...
  assert.equal(manifest['sap.app'].applicationVersion.version, '0.0.13')
  assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'app/bug-management-ui/package.json'))).version, '0.0.13')
  const appLock = JSON.parse(fs.readFileSync(path.join(root, 'app/bug-management-ui/package-lock.json')))
  assert.equal(appLock.version, '0.0.13')
  assert.equal(appLock.packages[''].version, '0.0.13')
  ...
  assert.match(index, /body#content\s*>\s*div:not\(#idtsAuthError\):not\(#idtsNotificationShellHost\):not\(#idtsProfileShellHost\)\s*\{[\s\S]*flex:\s*1 1 auto;[\s\S]*min-height:\s*0;/, 'UI5 component wrapper must consume only the viewport remaining below the notification toolbar')
