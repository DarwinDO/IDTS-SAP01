...
async function main () {
  ...
  assert.equal(manifest['sap.app'].applicationVersion.version, '0.0.12')
  assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'app/bug-management-ui/package.json'))).version, '0.0.12')
  const appLock = JSON.parse(fs.readFileSync(path.join(root, 'app/bug-management-ui/package-lock.json')))
  assert.equal(appLock.version, '0.0.12')
  assert.equal(appLock.packages[''].version, '0.0.12')
  ...
  assert.match(index, /#container\s*\{[\s\S]*flex:\s*1 1 auto;[\s\S]*min-height:\s*0;/, 'Fiori component must consume only the viewport remaining below the notification toolbar')
