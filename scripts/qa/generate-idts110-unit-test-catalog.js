...
const attachmentCases = [
  ...
  ['012', 'storage delete failure remains visible in the transactional outbox', 'NEGATIVE', 'BTP_INTEGRATION', 'BTP_REQUIRED', 'Inject a controlled S3 delete failure.', 'Metadata deletion is acknowledged, Bug workflow remains unchanged, and failed object cleanup stays in the persistent outbox with retry/error state instead of being silently lost.', source.attachmentBackend.concat(source.attachmentUi, source.attachmentStorage), ['NEGATIVE', 'PERSISTENCE']]
]
