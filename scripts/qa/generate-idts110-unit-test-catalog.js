...
add({ caseId: 'UT-AUTH-004', domain: 'Authentication', title: 'non-string password is rejected safely', classification: 'BOUNDARY', requirementIds: req('SRS-FR-AUTH-001'), input: 'Call login with a non-string password value.', expectedResult: 'HTTP 401 is returned without type details, stack trace, or session mutation.', sourceTrace: source.auth, coverage: ['BOUNDARY', 'SANITIZATION'] })
...
const requiredFields = [
  ...
  ['PRIORITY', 'priority_code'], ['SEVERITY', 'severity_code'], ['COMPONENT', 'applicationComponent_ID'], ['CATEGORY', 'defectCategory_ID'], ['REPORTER', 'reporter_ID']
]
...
const attachmentCases = [
  ...
  ['009', 'unauthenticated attachment write is denied', 'ROLE', 'BTP_INTEGRATION', 'BTP_REQUIRED', 'Upload without an authenticated IDTS role.', 'HTTP 401/403 is returned and existing attachment state is unchanged.', source.attachmentBackend, ['ROLE', 'PERSISTENCE']],
...
const notificationCases = [
  ...
  ['009', 'delivery before nextAttemptAt is not retried', 'PURE_UNIT', 'LOCAL', 'Evaluate a FAILED row whose nextAttemptAt is in the future.', 'The row is not claimed or sent.'],
  ['010', 'delivery at or after nextAttemptAt is retried', 'PURE_UNIT', 'LOCAL', 'Evaluate a FAILED row whose nextAttemptAt has elapsed.', 'The row becomes eligible and attempt count increments once.'],
  ['011', 'delivery at max attempts remains failed', 'PURE_UNIT', 'LOCAL', 'Evaluate a FAILED row whose attemptCount reached maxAttempts.', 'The row is not retried and remains FAILED.'],
  ['012', 'two workers cannot send the same delivery twice', 'BTP_INTEGRATION', 'BTP_REQUIRED', 'Start two workers against the same eligible delivery.', 'Only one compare-and-set claim succeeds and exactly one provider send occurs.'],
...
const aiCases = [
  ...
  ['026', 'PM reads allowlisted operational metrics only', 'ROLE', 'BTP_INTEGRATION', 'BTP_REQUIRED', 'Read 30-day metrics as PM.', 'Only aggregate capability/status/latency/review counts are returned; prompt/response/error/secret fields are absent.', source.aiReview],
  ['027', 'live provider rate limit preserves safe no-mutation fallback', 'BOUNDARY', 'BTP_INTEGRATION', 'BTP_REQUIRED', 'Trigger a controlled or naturally observed HTTP 429 within approved quota testing.', 'The documented cooldown/fallback result is recorded without retry storm or business mutation.', source.aiProvider]
]
