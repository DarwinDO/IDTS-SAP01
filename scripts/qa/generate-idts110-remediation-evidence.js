...
function resultSvg (testCase, result) {
  ...
  <rect x="56" y="48" width="1168" height="624" rx="24" fill="#ffffff" stroke="#14804a" stroke-width="4"/>
  <text x="104" y="120" font-family="Arial, sans-serif" font-size="30" font-weight="700" fill="#0b5e36">IDTS-110 LOCAL-PRIMARY RERUN</text>
  <text x="104" y="185" font-family="Arial, sans-serif" font-size="46" font-weight="700" fill="#162a22">${xml(testCase.caseId)} - CANDIDATE PASS</text>
  ...
  <text x="104" y="470" font-family="Arial, sans-serif" font-size="23" fill="#284b3b">All mapped suites exited 0. BTP confirmation is optional for the primary assertion.</text>
  <text x="104" y="520" font-family="Arial, sans-serif" font-size="23" fill="#284b3b">Final case acceptance remains with DonHV.</text>
...
for (const testCase of catalog.cases.filter(testCase => testCase.environment === 'HYBRID_BTP')) {
  ...
  if (!result || result.status !== 'CANDIDATE_PASS') throw new Error(`Missing passing suite result for ${testCase.caseId}`)
  ...
  Object.assign(manifest, {
    ...
    limitations: 'This is a local-primary candidate result from mapped domain suites. It is not BTP acceptance evidence; DonHV owns final case-level review and workbook integration.'
  })
