...
assert.match(loggedOutHtml, /class="primary-action"/)
assert.match(loggedOutHtml, /aria-labelledby="signed-out-title"/)
assert.match(loggedOutHtml, /@media \(max-width:/)
assert.doesNotMatch(loggedOutHtml, /<script|https?:\/\//i)
...
assert.match(demoReadiness, /\[string\]\$BrokerApp\s*=\s*'idts-user-access-broker'/)
assert.match(demoReadiness, /serviceStopped/)
assert.match(demoReadiness, /cf start/)
assert.match(demoReadiness, /\/ready/)
assert.match(demoReadiness, /BrokerRunning/)
assert.match(demoReadiness, /CoreBindingsReady/)
assert.match(demoReadiness, /BrokerBindingsReady/)
assert.match(demoReadiness, /AI binding:/)
assert.match(demoReadiness, /Notification services:/)
assert.match(demoReadiness, /Invitation config:/)
assert.match(demoReadiness, /Start-CfApp -Name \$BrokerApp/)
assert.doesNotMatch(demoReadiness, /Start-CfApp -Name ['"](?:idts-sap01-idts122-deployer|idts-sap01-migration-runner|idts-sap01-db-deployer)/)
