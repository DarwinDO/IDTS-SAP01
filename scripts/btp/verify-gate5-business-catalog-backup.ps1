...
try {
  ...
  $expected = @('sapModules', 'applicationComponents', 'defectCategories', 'componentCategories')
  ...
  Write-Output ("IDTS_GATE5_VERIFY_RESULT=PASS;TOTAL={0};COUNTS_B64={1};DIGEST_PREFIX={2}" -f $total, $countsBase64, $digest.Substring(0, 12))
  exit 0
} catch {
  Write-Output ("IDTS_GATE5_VERIFY_RESULT=FAIL;CODE={0}" -f $safeStage)
  exit 1
} finally {
