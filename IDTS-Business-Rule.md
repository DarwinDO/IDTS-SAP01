...
* An existing-user identity link targets one explicitly selected active legacy TESTER or DEVELOPER. The server derives the existing role, requires all external identity fields to be null, verifies the invited SAP email and immutable tuple, and uses provider readback only. On success it updates the same `Users.ID`; it must not create a replacement user or move existing Developer Profiles, responsibilities, Bugs, comments, notifications, or history.
* A legacy Developer without one exact matching `ACTIVE` immutable identity link is not eligible for a new direct assignment or Smart Assign result. Existing Bug assignees remain unchanged until a separate PM reassignment decision.
...
## BR-53 - Operations and audit must be safe, bounded, and state-valid

**English:** Only an active PM with `UserAdmin` may read the Operations and Audit actions. CAP must return explicit safe DTOs, apply stable ordering and server paging of 25 by default/100 maximum, and mask recipient/actor/target display values. Audit remains append-only and exposes only allowlisted action/result/state/summary fields plus a server-generated 12-character non-reversible correlation fingerprint. Readiness is derived from persisted safe outcome timestamps within a fixed seven-day freshness window: `sentAt` for `SENT`, `lastAttemptAt` for `FAILED`, and `completedAt` for successful access operations. Recent `SENT` is `AVAILABLE`; otherwise recent `FAILED` is `UNAVAILABLE`; `PENDING`, other, or inconclusive state is `UNKNOWN`. A delivery retry/list eligibility is valid only for a failed allowlisted transient row whose parent invitation remains `INVITED` and unexpired, with the expected `modifiedAt`, available retry budget, and no active lock; the reset and audit append share one transaction and the existing immediate outbox kick is registered only after commit. Existing access Retry/Reconcile guards remain authoritative: ambiguous outcomes permit Reconcile only, while permanent failures permit neither action. No raw email/provider content, provider ID, lock/lease/idempotency value, identity hash, token, credential, endpoint, or raw error may enter the API/UI/log/evidence.

**Tiếng Việt:** Chỉ PM active có `UserAdmin` mới được đọc các action Operations và Audit. CAP phải trả safe DTO explicit, order ổn định và paging server mặc định 25/tối đa 100, đồng thời mask recipient/actor/target display. Audit append-only và chỉ expose action/result/state/summary trong allowlist cùng correlation fingerprint 12 ký tự không đảo ngược do server tạo. Readiness chỉ derive từ timestamp outcome an toàn đã persist trong freshness window cố định bảy ngày: `sentAt` cho `SENT`, `lastAttemptAt` cho `FAILED`, và `completedAt` cho access operation thành công. `SENT` gần đây là `AVAILABLE`; nếu không có thì `FAILED` gần đây là `UNAVAILABLE`; PENDING/state khác/không kết luận là `UNKNOWN`. Eligibility retry delivery/list chỉ hợp lệ với row FAILED có transient code allowlist, parent invitation còn `INVITED` và chưa hết hạn, `modifiedAt` đúng, còn retry budget và không active lock; reset và audit append dùng cùng transaction, còn immediate outbox kick hiện có chỉ đăng ký sau commit. Guard Retry/Reconcile access hiện có vẫn là authority: outcome ambiguous chỉ được Reconcile, permanent failure không được action nào. Raw email/provider content, provider ID, lock/lease/idempotency, identity hash, token, credential, endpoint hoặc raw error không được vào API/UI/log/evidence.
