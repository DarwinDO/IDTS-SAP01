...
| User Administration | Controlled SAP identity invitation and verification, single-confirmation auto-queue for TESTER/DEVELOPER, second approval for PM/UserAdmin, versioned provisioning/role-change/revoke/suspend/reactivate operations, and one-time linking of an explicitly selected legacy TESTER/DEVELOPER to a verified SAP identity without replacing its internal row. Local `SUSPENDED`, exactly one PM/Tester/Developer business role, and optional `UserAdmin` only for selected PM users remain enforced. Signed invitation tokens are bounded and one-time; email links to official SAP account login/registration but IDTS does not query whether an email is registered. CAP owns operation/audit truth; a separate least-privilege broker owns external Role Collection reconciliation and read-only identity-link/reactivation proof. |
| Developer Administration | PM+UserAdmin configures Developer availability, workload, and Component Category responsibilities with optional SAP Module scope and PRIMARY/BACKUP/EXPERT level. Developer activation requires a complete profile after provider readback. Deactivation affects future matching only; existing Bug assignments are not automatically changed. |
...
## Business Catalog Administration scope

In scope: PM + UserAdmin administration of SAP Modules, Application Components, Defect Categories, and Component Category pairs with search, active filtering, add/edit, activate/deactivate, impact preview, ETag conflicts, and sanitized audit. Out of scope: hard delete, generic master-data editing, SAP/BTP/XSUAA/IAS administration, CSV import, schema browsing, and automatic Bug migration or reassignment.

Trong scope: PM + UserAdmin quản trị SAP Module, Application Component, Defect Category và cặp Component Category với search, filter active, add/edit, activate/deactivate, impact preview, ETag conflict và audit sanitize. Ngoài scope: hard delete, generic master-data editor, quản trị SAP/BTP/XSUAA/IAS, import CSV, schema browser và tự động migrate/reassign Bug.

## Operations and Audit usability scope

**English:** In scope: a PM + UserAdmin-only Operations workspace with Delivery and Provisioning subtabs, a separate append-only Audit tab, safe persisted-state readiness using a fixed seven-day freshness window, server pagination (25 default/100 maximum), stable ordering, status/type/action/date filters, masked display, safe detail dialogs, and state-valid delivery/access actions. Reuse the existing access Retry/Reconcile guards. Out of scope: provider/email outage simulation, endpoints, bindings, environment/credential inspection, log viewers, BTP/HANA/HDI controls, raw email/provider payloads, provider IDs, locks/leases/idempotency keys, identity hashes, tokens/JWTs, raw errors, arbitrary audit export, schema/index changes without measured evidence, and live provider/data mutation.

**Tiếng Việt:** Trong scope: workspace Operations chỉ cho PM + UserAdmin với subtab Delivery/Provisioning, tab Audit append-only riêng, readiness từ safe state đã persist với freshness window cố định bảy ngày, pagination server (mặc định 25/tối đa 100), order ổn định, filter status/type/action/date, display đã mask, dialog detail an toàn và action delivery/access đúng state. Dùng lại guard Retry/Reconcile access hiện có. Ngoài scope: mô phỏng outage provider/email, endpoint, binding, inspect environment/credential, log viewer, control BTP/HANA/HDI, raw email/provider payload, provider ID, lock/lease/idempotency key, identity hash, token/JWT, raw error, export audit tùy ý, schema/index khi chưa có measured evidence và live mutation provider/data.
