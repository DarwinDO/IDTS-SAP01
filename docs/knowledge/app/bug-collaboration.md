...
- IDTS overrides `Bugs.attachments.content` with `@Core.ContentDisposition.Type: 'attachment'`. The plugin facet remains standard, but selecting a filename must produce a browser download rather than an inline raw-content tab.
...
- IDTS override `Bugs.attachments.content` bằng `@Core.ContentDisposition.Type: 'attachment'`. Facet của plugin vẫn được giữ nguyên, nhưng chọn filename phải tải file thật thay vì mở tab raw-content inline.
