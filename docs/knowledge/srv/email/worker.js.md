...
`processEmailOutboxBatch()` giờ gọi `processNotificationDigestDeliveries()` sau processor Bug, invitation và access. Digest processor nhận cùng closure `sendMail` gắn với một sender của batch, cùng `now` và `workerID` của batch; batch config được clamp về bound HANA-safe 100 của digest. Processor chỉ claim và gửi snapshot `NotificationDigestDeliveries` đã lưu, yêu cầu `digestType` persona allowlist khớp role hiện tại và chỉ đếm profile active khi revalidate; retry không gọi lại `buildDigestSnapshot()`. Batch vẫn tạo một sender và close một lần trong `finally`, nên digest không thêm connection provider, timer hoặc worker thứ hai.
