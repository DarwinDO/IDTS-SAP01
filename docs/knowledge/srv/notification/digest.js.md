...
`processNotificationDigestDeliveries({ tx, config, sendMail, now, workerID })` theo đúng pattern outbox hiện có: batch config được clamp tối đa 100 theo bound HANA-safe, candidate read và recipient `IN` dùng bound đó, claim compare-and-set bằng lock token, ghi thời điểm attempt, gọi provider một lần qua `sendMail` chung được inject, map lỗi transport an toàn, backoff, rồi cập nhật `SENT`/`FAILED`/`SKIPPED`. Processor gửi body đã lưu thay vì dựng snapshot mới. Row có business date tương lai không gửi sớm; persona role/profile hiện tại được revalidate trước send, recipient inactive/sai persona/email bị skip an toàn; lỗi không tạo failure notification cho end user.
