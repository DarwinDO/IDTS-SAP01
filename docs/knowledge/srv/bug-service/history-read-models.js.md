...
Backend helper module for `history-read-models` behavior inside BugService. Read this file as one focused part of the service runtime. It is loaded directly or indirectly from `srv/service.js` and supports validation, permissions, read enrichment, side effects, drafts, content, monitoring, or history.
...
- Line 5: `const { trimToNull } = require('./helpers')` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 49: `function ensureHistoryEventSelectDependencies (req) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 67: `async function enrichHistoryEventPayload (events, req, entities) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 112: `function normalizeHistoryLogs (logs) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 118: `function compareHistoryLogs (left, right) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 133: `function historyFieldOrderIndex (fieldName) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 138: `function summarizeHistoryLogs (logs) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 149: `function formatHistoryLogFragment (log) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 189: `function historyDisplayValue (displayValue, rawValue) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
...
Backend helper module for `history-read-models` behavior inside BugService. File này nằm ở lớp backend CAP service. Nó xử lý request OData, kiểm tra nghiệp vụ, tính field hiển thị, phân quyền hoặc side effect.
...
- Source layer: `srv`
- Source type: `.js`
- Source line count at documentation time: 203
