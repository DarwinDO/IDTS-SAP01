...
Backend helper module for `monitoring` behavior inside BugService. Read this file as one focused part of the service runtime. It is loaded directly or indirectly from `srv/service.js` and supports validation, permissions, read enrichment, side effects, drafts, content, monitoring, or history.
...
- Line 8: `} = require('./constants')` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 10: `const { trimToNull } = require('./helpers')` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 23: `async function readDeveloperWorkloads (req, entities) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 80: `function buildDeveloperWorkloadRows (profiles, bugs) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 137: `function emptyDeveloperWorkloadRow (base) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 164: `function isOverdueBug (bug) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 168: `function isCurrentDeveloperAction (row, bug) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 175: `function todayDateString () {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
- Line 179: `function decimalToNumber (value) {` — This is a control point that changes imports, metadata, runtime behavior, routing, test behavior, or displayed text.
...
Backend helper module for `monitoring` behavior inside BugService. File này nằm ở lớp backend CAP service. Nó xử lý request OData, kiểm tra nghiệp vụ, tính field hiển thị, phân quyền hoặc side effect.
...
- Source layer: `srv`
- Source type: `.js`
- Source line count at documentation time: 371
