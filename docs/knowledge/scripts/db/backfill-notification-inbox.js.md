...
This controlled helper indexes eligible Bug notifications from the previous 30 days up to the frozen run timestamp. Existing email deliveries do not disqualify a source: the rule is to create no new email. It excludes only already-indexed sources, creates no access entry, email delivery or domain event, and prints counts rather than recipient data. Source reads use 500-row keyset batches and future timestamps are excluded. Running without arguments is dry-run only and ends with `No database was changed`. The `--execute` path exists for a later separately approved migration gate and inserts the missing rows in one transaction.

### Important source anchors

- **Location**: `buildBugInboxBackfillPlan`. **Concept**: bounded, idempotent Bug-only candidate plan. **Impact if broken**: eligible historical events can disappear, future rows can be included, or duplicate inbox rows can be created. **Check together**: schema unique constraints and backfill QA.
...
Helper có kiểm soát này index Bug notification trong 30 ngày gần nhất tới timestamp đã freeze. Email delivery có sẵn không làm source mất điều kiện: quy định là không tạo email mới. Helper chỉ loại source đã index, không tạo access entry, email delivery hoặc domain event và chỉ in count, không in dữ liệu người nhận. Source được đọc theo keyset batch 500 row và loại timestamp tương lai. Chạy không có argument chỉ dry-run và kết thúc bằng `No database was changed`. Nhánh `--execute` dành cho migration gate sau được duyệt riêng và insert row thiếu trong một transaction.

### Các điểm neo quan trọng

- **Vị trí**: `buildBugInboxBackfillPlan`. **Khái niệm**: plan candidate Bug-only bounded và idempotent. **Ảnh hưởng nếu sai**: event hợp lệ có thể bị bỏ sót, row tương lai bị đưa vào hoặc inbox row bị trùng. **Kiểm tra cùng**: unique constraint schema và QA backfill.
