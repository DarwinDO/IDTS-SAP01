# Knowledge: `db/data/idts.cap-AvailabilityStatuses.csv`

## English

### What this file is for

Seed dataset for `AvailabilityStatuses`. Read this file as local/demo data that CAP loads into the database. It affects dropdowns, value helps, sample bugs, workflow labels, and demo behavior.
...
- During local database deployment, CAP imports this CSV into `AvailabilityStatuses`.
...
- This CSV loads `3` seed rows into `AvailabilityStatuses`.
...
- Line 2: `AVAILABLE, Available, Developer can receive assignments, 10, true, 3` — The first data row is a concrete example loaded into the local development database.
...
- **CSV filename/entity → `db/schema.cds`**: CAP loads this file into the `AvailabilityStatuses` entity. Impact: Column names and foreign keys must match the schema.
- **Seeded values → `srv/service.cds`**: BugService exposes or uses `AvailabilityStatuses` directly or indirectly. Impact: Changing seed codes/names can change dropdowns, workflow matching, status labels, and demo data.
...
Seed dataset for `AvailabilityStatuses`. File này nằm ở lớp dữ liệu. Nó định nghĩa schema hoặc seed data mà service và Fiori dùng thông qua CAP/OData.
...
- Source file: `db/data/idts.cap-AvailabilityStatuses.csv`
- Knowledge mirror: `docs/knowledge/db/data/idts.cap-AvailabilityStatuses.csv.md`
- Source layer: `db`
- Source type: `.csv`
