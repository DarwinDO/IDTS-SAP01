...
Seed dataset for `DeveloperResponsibilities`. Read this file as local/demo data that CAP loads into the database. It affects dropdowns, value helps, sample bugs, workflow labels, and demo behavior.
...
- During local database deployment, CAP imports this CSV into `DeveloperResponsibilities`.
...
- This CSV loads `8` seed rows into `DeveloperResponsibilities`.
- Header columns: `ID, developerProfile_ID, componentCategory_ID, sapModule_ID, responsibilityLevel_code, active`.
...
- Line 1: `ID, developerProfile_ID, componentCategory_ID, sapModule_ID, responsibilityLevel_code, active` — The header defines the columns CAP imports into the matching seed entity.
- Line 2: `70000000-0000-0000-0000-000000000001, 20000000-0000-0000-0000-000000000001, 60000000-0000-0000-0000-000000000002, , PRIMARY, true` — The first data row is a concrete example loaded into the local development database.
...
- **CSV filename/entity → `db/schema.cds`**: CAP loads this file into the `DeveloperResponsibilities` entity. Impact: Column names and foreign keys must match the schema.
- **Seeded values → `srv/service.cds`**: BugService exposes or uses `DeveloperResponsibilities` directly or indirectly. Impact: Changing seed codes/names can change dropdowns, workflow matching, status labels, and demo data.
...
Seed dataset for `DeveloperResponsibilities`. File này nằm ở lớp dữ liệu. Nó định nghĩa schema hoặc seed data mà service và Fiori dùng thông qua CAP/OData.
...
- Source layer: `db`
- Source type: `.csv`
- Source line count at documentation time: 9
