...
Seed dataset for `DeveloperProfiles`. Read this file as local/demo data that CAP loads into the database. It affects dropdowns, value helps, sample bugs, workflow labels, and demo behavior.
...
- During local database deployment, CAP imports this CSV into `DeveloperProfiles`.
...
- This CSV loads `2` seed rows into `DeveloperProfiles`.
- Header columns: `ID, user_ID, availabilityStatus_code, workloadLimit, active`.
...
- Line 1: `ID, user_ID, availabilityStatus_code, workloadLimit, active` — The header defines the columns CAP imports into the matching seed entity.
- Line 2: `20000000-0000-0000-0000-000000000001, 10000000-0000-0000-0000-000000000002, AVAILABLE, 5, true` — The first data row is a concrete example loaded into the local development database.
...
- **CSV filename/entity → `db/schema.cds`**: CAP loads this file into the `DeveloperProfiles` entity. Impact: Column names and foreign keys must match the schema.
- **Seeded values → `srv/service.cds`**: BugService exposes or uses `DeveloperProfiles` directly or indirectly. Impact: Changing seed codes/names can change dropdowns, workflow matching, status labels, and demo data.
...
Seed dataset for `DeveloperProfiles`. File này nằm ở lớp dữ liệu. Nó định nghĩa schema hoặc seed data mà service và Fiori dùng thông qua CAP/OData.
...
- Source file: `db/data/idts.cap-DeveloperProfiles.csv`
- Knowledge mirror: `docs/knowledge/db/data/idts.cap-DeveloperProfiles.csv.md`
- Source layer: `db`
- Source type: `.csv`
- Source line count at documentation time: 3
