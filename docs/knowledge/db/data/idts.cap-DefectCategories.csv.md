# Knowledge: `db/data/idts.cap-DefectCategories.csv`

## English

### What this file is for

Seed dataset for `DefectCategories`. Read this file as local/demo data that CAP loads into the database. It affects dropdowns, value helps, sample bugs, workflow labels, and demo behavior.
...
- During local database deployment, CAP imports this CSV into `DefectCategories`.
...
- This CSV loads `8` seed rows into `DefectCategories`.
- Header columns: `ID, code, name, categoryType, active`.
...
- Line 1: `ID, code, name, categoryType, active` — The header defines the columns CAP imports into the matching seed entity.
- Line 2: `50000000-0000-0000-0000-000000000001, FIORI_UI5, SAP Fiori UI5, Technical, true` — The first data row is a concrete example loaded into the local development database.
...
- **CSV filename/entity → `db/schema.cds`**: CAP loads this file into the `DefectCategories` entity. Impact: Column names and foreign keys must match the schema.
- **Seeded values → `srv/service.cds`**: BugService exposes or uses `DefectCategories` directly or indirectly. Impact: Changing seed codes/names can change dropdowns, workflow matching, status labels, and demo data.
...
Seed dataset for `DefectCategories`. File này nằm ở lớp dữ liệu. Nó định nghĩa schema hoặc seed data mà service và Fiori dùng thông qua CAP/OData.
...
- Source file: `db/data/idts.cap-DefectCategories.csv`
- Knowledge mirror: `docs/knowledge/db/data/idts.cap-DefectCategories.csv.md`
- Source layer: `db`
- Source type: `.csv`
