# Knowledge: `db/data/idts.cap-ProcessorRoleValues.csv`

## English

### What this file is for

Seed dataset for `ProcessorRoleValues`. Read this file as local/demo data that CAP loads into the database. It affects dropdowns, value helps, sample bugs, workflow labels, and demo behavior.
...
- During local database deployment, CAP imports this CSV into `ProcessorRoleValues`.
...
- This CSV loads `5` seed rows into `ProcessorRoleValues`.
...
- Line 2: `TESTER, Tester, Tester is expected to take the next action, 10, true, 1` — The first data row is a concrete example loaded into the local development database.
...
- **CSV filename/entity → `db/schema.cds`**: CAP loads this file into the `ProcessorRoleValues` entity. Impact: Column names and foreign keys must match the schema.
- **Seeded values → `srv/service.cds`**: BugService exposes or uses `ProcessorRoleValues` directly or indirectly. Impact: Changing seed codes/names can change dropdowns, workflow matching, status labels, and demo data.
...
Seed dataset for `ProcessorRoleValues`. File này nằm ở lớp dữ liệu. Nó định nghĩa schema hoặc seed data mà service và Fiori dùng thông qua CAP/OData.
...
- Source file: `db/data/idts.cap-ProcessorRoleValues.csv`
- Knowledge mirror: `docs/knowledge/db/data/idts.cap-ProcessorRoleValues.csv.md`
- Source layer: `db`
- Source type: `.csv`
- Source line count at documentation time: 6
