...
Seed dataset for `ApplicationComponents`. Read this file as local/demo data that CAP loads into the database. It affects dropdowns, value helps, sample bugs, workflow labels, and demo behavior.
...
- During local database deployment, CAP imports this CSV into `ApplicationComponents`.
...
- This CSV loads `7` seed rows into `ApplicationComponents`.
- Header columns: `ID, code, name, componentType, active`.
...
- Line 1: `ID, code, name, componentType, active` — The header defines the columns CAP imports into the matching seed entity.
- Line 2: `40000000-0000-0000-0000-000000000001, IDTS_BUG_REPORT, IDTS Bug Report, IDTS_APP, true` — The first data row is a concrete example loaded into the local development database.
...
- **CSV filename/entity → `db/schema.cds`**: CAP loads this file into the `ApplicationComponents` entity. Impact: Column names and foreign keys must match the schema.
- **Seeded values → `srv/service.cds`**: BugService exposes or uses `ApplicationComponents` directly or indirectly. Impact: Changing seed codes/names can change dropdowns, workflow matching, status labels, and demo data.
...
Seed dataset for `ApplicationComponents`. File này nằm ở lớp dữ liệu. Nó định nghĩa schema hoặc seed data mà service và Fiori dùng thông qua CAP/OData.
...
- Source layer: `db`
- Source type: `.csv`
- Source line count at documentation time: 8
