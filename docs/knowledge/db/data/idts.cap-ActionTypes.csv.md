...
Seed dataset for `ActionTypes`. Read this file as local/demo data that CAP loads into the database. It affects dropdowns, value helps, sample bugs, workflow labels, and demo behavior.
...
- During local database deployment, CAP imports this CSV into `ActionTypes`.
...
- This CSV loads `11` seed rows into `ActionTypes`.
...
- Line 2: `CREATE, Create, Bug was created, 10, true, 0` — The first data row is a concrete example loaded into the local development database.
...
- **CSV filename/entity → `db/schema.cds`**: CAP loads this file into the `ActionTypes` entity. Impact: Column names and foreign keys must match the schema.
- **Seeded values → `srv/service.cds`**: BugService exposes or uses `ActionTypes` directly or indirectly. Impact: Changing seed codes/names can change dropdowns, workflow matching, status labels, and demo data.
...
Seed dataset for `ActionTypes`. File này nằm ở lớp dữ liệu. Nó định nghĩa schema hoặc seed data mà service và Fiori dùng thông qua CAP/OData.
...
- Source file: `db/data/idts.cap-ActionTypes.csv`
- Knowledge mirror: `docs/knowledge/db/data/idts.cap-ActionTypes.csv.md`
- Source layer: `db`
- Source type: `.csv`
- Source line count at documentation time: 12
