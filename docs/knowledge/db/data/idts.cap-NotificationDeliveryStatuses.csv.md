# Knowledge: `db/data/idts.cap-NotificationDeliveryStatuses.csv`

## English

### What this file is for

Seed dataset for `NotificationDeliveryStatuses`. Read this file as local/demo data that CAP loads into the database. It affects dropdowns, value helps, sample bugs, workflow labels, and demo behavior.
...
- During local database deployment, CAP imports this CSV into `NotificationDeliveryStatuses`.
...
- This CSV loads `4` seed rows into `NotificationDeliveryStatuses`.
...
- Line 2: `PENDING, Pending, Notification is pending delivery, 10, true, 1` — The first data row is a concrete example loaded into the local development database.
...
- **CSV filename/entity → `db/schema.cds`**: CAP loads this file into the `NotificationDeliveryStatuses` entity. Impact: Column names and foreign keys must match the schema.
- **Seeded values → `srv/service.cds`**: BugService exposes or uses `NotificationDeliveryStatuses` directly or indirectly. Impact: Changing seed codes/names can change dropdowns, workflow matching, status labels, and demo data.
...
Seed dataset for `NotificationDeliveryStatuses`. File này nằm ở lớp dữ liệu. Nó định nghĩa schema hoặc seed data mà service và Fiori dùng thông qua CAP/OData.
...
- Source file: `db/data/idts.cap-NotificationDeliveryStatuses.csv`
- Knowledge mirror: `docs/knowledge/db/data/idts.cap-NotificationDeliveryStatuses.csv.md`
- Source layer: `db`
- Source type: `.csv`
