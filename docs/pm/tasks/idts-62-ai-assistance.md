...
- IDTS-71: In progress on branch `feature/idts-71-ai-security-review-donhv`. The security review found and fixed one backend hardening gap: unsafe provider output in classification reasons is now marked `AI_OUTPUT_UNSAFE` and replaced with safe review/fallback copy. New focused scripts are `qa:idts71:programmatic` and `qa:idts71:render-smoke`.
...
- IDTS-71: Dang lam tren branch `feature/idts-71-ai-security-review-donhv`. Vong review security da tim va fix mot gap hardening backend: output provider khong an toan trong classification reason bay gio duoc danh dau `AI_OUTPUT_UNSAFE` va thay bang copy review/fallback an toan. Script focused moi la `qa:idts71:programmatic` va `qa:idts71:render-smoke`.
