...
- IDTS-76: In progress. The local Object Page implementation reuses the existing backend `summarizeBugHandoff` action and shared `AiReviewUi`, adds a review-only Handoff Summary section/dialog, and has local evidence for positive, sparse-data, unsafe-output, safe-failure, and no-workflow-mutation scenarios. PR, Render deploy, and shared-QA evidence are still pending.
...
- IDTS-76: Dang lam. Implementation Object Page local tai su dung backend action `summarizeBugHandoff` va shared `AiReviewUi`, them section/dialog Handoff Summary chi de review, va da co evidence local cho positive, sparse-data, unsafe-output, safe-failure va no-workflow-mutation. PR, Render deploy va shared-QA evidence van con pending.
