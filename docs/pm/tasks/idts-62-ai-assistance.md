...
- IDTS-72: Final QA acceptance was reopened for visual evidence after DonHV observed that the visible UI evidence only clearly covered Smart Assign. Fresh local acceptance passed 6/6 suites, and the visual evidence audit now lives under `docs/pm/evidence/idts-72/visual-ai-flows/`. PR #126 merged that evidence into `dev`. The audit confirms Smart Assign has product UI evidence, while duplicate detection, classification suggestion, and handoff summary are currently backend/API verified but not yet exposed as standalone product UI panels. Closure now depends on accepting API-level evidence for those three flows or completing UI follow-up tasks.
- IDTS-74: Implemented on branch `feature/idts-74-duplicate-ai-review-panel-donhv`. The Object Page now has a `Find Similar Bugs` review entry point that calls the existing `suggestSimilarBugs` action through `DuplicateReview.js`, reuses `AiReviewUi`, and remains review-only with no automatic `DuplicateLinks` write. Local focused QA passed 133 checks; browser smoke captured the real dialog, candidate, safe copy, and manual-review guidance. Final PR is pending.
- IDTS-75: Created under Epic `IDTS-62` as the AI classification suggestion review panel follow-up. It blocks full visual closure of `IDTS-72` and relates to backend task `IDTS-67`.
...
- IDTS-72: Final QA acceptance da duoc reopen de bo sung visual evidence sau khi DonHV thay evidence UI nhin thay ro moi cover Smart Assign. Fresh local acceptance pass 6/6 suite, va evidence audit moi nam trong `docs/pm/evidence/idts-72/visual-ai-flows/`. PR #126 da merge evidence nay vao `dev`. Audit xac nhan Smart Assign co product UI evidence, con duplicate detection, classification suggestion va handoff summary hien da verify bang backend/API nhung chua expose thanh panel UI san pham rieng. Viec dong task phu thuoc vao quyet dinh chap nhan API-level evidence cho ba luong nay hoac hoan tat cac task UI follow-up.
- IDTS-74: Da implement tren branch `feature/idts-74-duplicate-ai-review-panel-donhv`. Object Page co entry `Find Similar Bugs` de review, goi action da co `suggestSimilarBugs` qua `DuplicateReview.js`, reuse `AiReviewUi`, va van chi la review-only, khong tu ghi `DuplicateLinks`. Focused QA local pass 133 checks; browser smoke da chup dialog that, candidate, copy an toan va huong dan user tu review. PR cuoi con pending.
- IDTS-75: Da tao duoi Epic `IDTS-62` lam follow-up panel review AI classification suggestion. Task nay block viec dong `IDTS-72` theo tieu chi visual day du va relate voi backend task `IDTS-67`.
