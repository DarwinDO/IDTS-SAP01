...
- IDTS-75: In Progress on `feature/idts-75-classification-ai-review-panel-donhv`. The Object Page now has a `Classification Assistance` section and review-only dialog that reuses `suggestClassification`, compares current/suggested catalog values, marks invalid and low-confidence rows, and performs no Bug write. Local static QA passes 106 checks and browser smoke passes seven checks covering PM/Tester access plus positive/guarded/failure/no-mutation behavior. PR, Render deployment, and shared-QA evidence remain.
...
- IDTS-72: Final QA acceptance da duoc reopen de bo sung visual evidence sau khi DonHV thay evidence UI nhin thay ro moi cover Smart Assign. Fresh local acceptance pass 6/6 suite, va evidence audit moi nam trong `docs/pm/evidence/idts-72/visual-ai-flows/`. PR #126 da merge evidence nay vao `dev`. IDTS-74 hien da co evidence UI san pham tren ban deploy cho duplicate/similar review. Viec dong task van phu thuoc vao hoan tat `IDTS-75` va `IDTS-76`, hoac chap nhan ro rang API-level evidence cho classification suggestion va handoff summary.
...
- IDTS-75: In Progress tren `feature/idts-75-classification-ai-review-panel-donhv`. Object Page co section `Classification Assistance` va dialog review-only tai su dung `suggestClassification`, so sanh current/suggested catalog value, danh dau invalid/low-confidence va khong ghi vao Bug. Static QA local pass 106 check va browser smoke pass bay check gom PM/Tester access, positive/guarded/failure/no-mutation. Con PR, Render deploy va shared-QA evidence.
