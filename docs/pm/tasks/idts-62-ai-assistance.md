...
- IDTS-68: In Progress. DonHV is implementing on behalf of SangVN on branch `feature/idts-68-grounded-bug-summary-donhv`. Backend now has a draft `summarizeBugHandoff` action, grounded bug/comment/history reader, provider/fallback handling, sanitized `BUG_SUMMARY` audit, and focused QA evidence `28/0`. It still needs full regression, PR, CI, merge, Jira evidence, and closure.
...
- IDTS-68: Dang In Progress. DonHV dang implement thay SangVN tren branch `feature/idts-68-grounded-bug-summary-donhv`. Backend hien co ban nhap action `summarizeBugHandoff`, doc du lieu bug/comment/history co can cu, xu ly provider/fallback, audit `BUG_SUMMARY` da sanitize, va focused QA evidence `28/0`. Van can chay full regression, tao PR, cho CI, merge, cap nhat Jira evidence va closure.
