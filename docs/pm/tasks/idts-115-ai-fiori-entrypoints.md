...
- Code commit `212bde0` is pushed on `fix/idts-115-classification-apply-feedback-donhv`; PR #311 targets `dev`. The first remote QA Depth run passed. This status-only handoff update must receive its own fresh exact-head PASS; no merge or deployment is performed in this session.
