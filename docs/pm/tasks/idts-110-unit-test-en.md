...
- Case 13 now has bounded live Chrome evidence using an authorized disposable BTP shadow identity. Chrome DevTools captured the app `AuthService.me` request as HTTP `403 application/json`, the exact safe access-denied UI rendered, and the service rejects a missing active external-identity mapping with `403`. The temporary role was removed and BTP readback has zero role collections; no IDTS business mutation was issued.
- The current evidence-card model is `278 PASS / 0 BLOCKED / 278 total`. The Unit Test workbook and Drive copy are unchanged; their prior receipt is intentionally not reissued in this evidence-only gate.
...
- Case 13 nay có evidence Chrome live giới hạn bằng một BTP shadow identity disposable đã được duyệt. Chrome DevTools đã capture request `AuthService.me` của app là HTTP `403 application/json`, UI access-denied chính xác đã render, và service từ chối mapping external identity active bị thiếu bằng `403`. Role tạm đã được gỡ và BTP readback không còn role collection; không có business mutation IDTS nào được thực hiện.
- Mô hình evidence card hiện tại là `278 PASS / 0 BLOCKED / 278 total`. Workbook Unit Test và bản Drive không thay đổi; receipt cũ của chúng được chủ động không phát hành lại trong gate chỉ cập nhật evidence này.
