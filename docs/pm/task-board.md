...
| IDTS-89 | Trace each Bug workflow OData action with a dedicated audit ActionType | DonHV / NhanT support | Blocked for reviewed runtime deployment. Shared QA PostgreSQL UPSERT/readback is complete with 11/11 exact codes and preserved business/history/user/legacy rows. Live Render commit `07be39e` is stale, so exact workflow smoke remains pending; never substitute broad `cds-deploy`. |
...
| IDTS-89 | Trace 1-1 OData workflow action bang audit ActionType rieng | DonHV / NhanT ho tro | Bi chan o deploy runtime da review. PostgreSQL Shared QA da UPSERT/readback du 11/11 code va giu nguyen Bugs/History/Users/legacy rows. Render live commit `07be39e` dang cu nen exact workflow smoke con pending; khong thay bang `cds-deploy` rong. |
