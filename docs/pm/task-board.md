...
| IDTS-97 | Add privacy-safe operational metrics for AI assistance | SangVN | Local implementation complete on `feature/idts-97-ai-operational-metrics-sangvn`: safe status/latency emission, four feature audit integrations, PM aggregate, 274/0 focused/regression checks, CAP compile, and secret scan pass. Await push/review and deferred Knowledge Gate; keep In Progress. |
...
| IDTS-97 | Them operational metrics an toan cho AI assistance | SangVN | Implementation local da xong tren `feature/idts-97-ai-operational-metrics-sangvn`: emit status/latency an toan, tich hop bon feature audit, PM aggregate, focused/regression `274/0`, CAP compile va secret scan pass. Cho push/review va Knowledge Gate dang hoan; giu In Progress. |
