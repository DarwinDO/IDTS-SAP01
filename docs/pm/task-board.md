...
| IDTS-89 | Trace each Bug workflow OData action with a dedicated audit ActionType | DonHV / NhanT support | Runtime release is live after Knowledge Gate 90% PASS and normal protected merge of PR #163. Deploy `dep-d9gtkhrrjlhs73d4mhqg` runs exact merge SHA `97792e8`; health, database preservation, 11/11 exact codes, 11/11 legacy codes, and error-log checks pass. Auto-deploy stays off and pre-deploy stays `true`. Awaiting authenticated exact-action Shared QA smoke; Jira remains In Progress. |
...
| IDTS-89 | Trace 1-1 OData workflow action bang audit ActionType rieng | DonHV / NhanT ho tro | Runtime da live sau Knowledge Gate 90% PASS va merge PR #163 dung branch protection. Deploy `dep-d9gtkhrrjlhs73d4mhqg` chay dung merge SHA `97792e8`; health, bao toan database, 11/11 exact code, 11/11 legacy code va error-log deu pass. Auto-deploy van tat, pre-deploy van la `true`. Con cho authenticated exact-action smoke tren Shared QA; Jira giu In Progress. |
