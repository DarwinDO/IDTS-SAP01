...
| IDTS-114 | Backend: Integrate staged Vercel AI Gateway into SAP BTP AI assistance | DonHV | In Progress: the three previously missing UI entry points are now deployed through IDTS-115. PM browser review/apply/confirm/metrics and Qwen embedding success are evidenced, but Qwen structured primary still records provider-error fallback for Classification, Handoff and Smart Assign. Tester/Developer interactive identity evidence remains pending. Evidence: `docs/pm/evidence/idts-114/btp-browser/` and `docs/pm/evidence/idts-115/`. |
