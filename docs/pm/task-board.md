...
| IDTS-114 | Backend: Integrate staged Vercel AI Gateway into SAP BTP AI assistance | DonHV | In Progress: feature-specific routing is Classification `openai/gpt-5.4-nano`, Handoff `minimax/minimax-m2.5` with one Grok backup, Smart Assign `zai/glm-4.7-flash`, and Similar Bugs Qwen embedding. PM browser/audit acceptance passed for all four providers, but Smart Assign output was discarded by a safety false positive on the ordinary word `select`; the contextual SQL correction passes local regressions and awaits normal merge/selective BTP service rollout. Tester/Developer role evidence remains deferred. Evidence: `docs/pm/evidence/idts-114/`. |
