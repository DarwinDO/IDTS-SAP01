...
| IDTS-89 | Trace each Bug workflow OData action with a dedicated audit ActionType | DonHV / NhanT support | Source implementation ready; database narrow UPSERT complete; runtime deploy blocked by Knowledge Gate and unsafe broad pre-deploy behavior. PR #163 remains Draft. Runtime-only release plan prepared, not approved/executed. |
...
| IDTS-89 | Trace 1-1 OData workflow action bang audit ActionType rieng | DonHV / NhanT ho tro | Source implementation san sang; narrow UPSERT database da xong; runtime deploy bi chan boi Knowledge Gate va broad pre-deploy khong an toan. PR #163 giu Draft. Release plan runtime-only da soan, chua duyet/chua chay. |
