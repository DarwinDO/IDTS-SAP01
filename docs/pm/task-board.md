...
| IDTS-103 | Formalize Functional and Technical Specification tables and Vietnamese parity | DonHV | In Progress: four local workbook candidates are regenerated with formal one-record-per-row tables and shared catalogs. Local structural/visual checks pass; final gates, DonHV review, PR, and Drive same-ID synchronization remain pending. |
...
| IDTS-103 | Chuan hoa bang trong Functional va Technical Specification va dong deu ban VI | DonHV | Dang lam: bon workbook local da duoc regenerate bang bang formal, moi record mot dong va catalog dung chung. Kiem tra cau truc/visual local da pass; con final gate, DonHV review, PR va dong bo Drive giu nguyen ID. |
