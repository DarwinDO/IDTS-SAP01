...
| IDTS-114 | Backend: Integrate staged Vercel AI Gateway into SAP BTP AI assistance | DonHV | In Progress: PR #209 is merged; Qwen structured/embedding primary and bounded OpenAI fallbacks are live on SAP BTP. Provider tasks and 169 local regression checks pass with no mutation. Ling plain chat works but JSON-Schema structured output returned HTTP 400. Remaining: authenticated browser smoke of all four AI actions. |
