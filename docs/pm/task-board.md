...
| IDTS-114 | Backend: Integrate staged Vercel AI Gateway into SAP BTP AI assistance | DonHV | In Progress: fresh PM sequential acceptance records Qwen primary SUCCESS for Classification, Handoff, Smart Assign and Similar Bugs. Smart Assign exposed an AppRouter HTTP 504 because Qwen completed at 30,495 ms after the roughly 30-second route boundary; a tested 24-second feature deadline now returns deterministic explanations before that boundary and awaits selective BTP rollout/retest. Responsive UI polish and Tester/Developer evidence remain pending. Evidence: `docs/pm/evidence/idts-114/`. |
| IDTS-115 | FE: Expose existing AI actions in supported Fiori dialogs and PM Dashboard | DonHV | In Progress: PR #214 merged and selective SAP BTP deployment at `ae209c8f` is live. PM Apply Classification, Confirm Duplicate, AI Activity, reload and HANA duplicate readback PASS. Remaining gates are Tester/Developer member-owned browser role evidence and follow-up of create-flow binding console errors. Evidence: `docs/pm/evidence/idts-115/`. |
