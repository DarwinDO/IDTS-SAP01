...
| IDTS-114 | Backend: Integrate staged Vercel AI Gateway into SAP BTP AI assistance | DonHV | In Progress: PR #211 merged at `016a6067`; Qwen structured/embedding primary and bounded OpenAI fallbacks are live on SAP BTP, with 169 local regression checks passing. PM browser acceptance on `BUG-0018` verified visible Similar Bugs, Classification, Handoff Summary and Smart Assign review/no-mutation flows, but stable provider success is not proven after sanitized gateway 429/400 responses. Apply Classification, Confirm Duplicate and Operational Metrics have no deployed UI entry point; Tester/Developer role evidence is still blocked on member-owned SAP identities. Evidence: `docs/pm/evidence/idts-114/btp-browser/`. |
| IDTS-115 | FE: Expose existing AI actions in supported Fiori dialogs and PM Dashboard | DonHV | In Progress: fresh branch from `origin/dev` merge SHA `b19158c9`; Apply Classification, Confirm Duplicate and PM-only AI Activity are being implemented without changing CAP contracts or workflow behavior. Local focused acceptance and BTP deployment remain pending. |
