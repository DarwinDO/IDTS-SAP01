...
| IDTS-114 | Backend: Integrate staged Vercel AI Gateway into SAP BTP AI assistance | DonHV | In Progress: native-fetch Vercel Gateway adapter, Ling-first configuration, bounded Qwen-to-OpenAI fallback and safe model telemetry are implemented. Deterministic provider 15/15 and existing AI regressions pass. Live Ling is intentionally pending a private BTP key after PR merge; no secret is committed. |
