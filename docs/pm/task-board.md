...
| IDTS-79 | Mentor-ready Shared QA acceptance | DonHV with SangVN, DatDT, NhanT support | Baseline/API/AI UI pass is recorded; fresh PM/Developer/Tester lifecycle, S3 persistence across a Render restart, delivery `SENT`, and connected Inbox receipt also passed. Full acceptance remains blocked by `IDTS-81`: a new email fallback deep link still uses the legacy Fiori route. Evidence: `docs/pm/evidence/idts-79/mentor-ready-qa-report.vi.md`. |
...
| IDTS-79 | Acceptance Shared QA truoc mentor review | DonHV, SangVN, DatDT, NhanT | Da co baseline/API/AI UI va fresh PM/Developer/Tester lifecycle, S3 persistence sau Render restart, delivery `SENT`, va Inbox receipt. Full acceptance con bi block boi `IDTS-81`: fallback deep link trong email moi van dung Fiori route cu. Evidence: `docs/pm/evidence/idts-79/mentor-ready-qa-report.vi.md`. |
