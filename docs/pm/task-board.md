...
| IDTS-114 | Backend: Integrate staged Vercel AI Gateway into SAP BTP AI assistance | DonHV | In Progress: feature-specific routing is Classification `openai/gpt-5.4-nano`, Handoff `minimax/minimax-m2.5` with one Grok backup, Smart Assign `zai/glm-4.7-flash`, and Similar Bugs Qwen embedding. PR #251 and selective BTP service rollout resolved the Smart Assign output-safety false positive; PM browser acceptance now shows AI-generated explanations for all three candidates with no mutation. Tester/Developer role evidence remains deferred. Evidence: `docs/pm/evidence/idts-114/`. |
| IDTS-115 | FE: Expose existing AI actions in supported Fiori dialogs and PM Dashboard | DonHV | In Progress: PM Apply Classification, Confirm Duplicate, AI Activity and create-flow persistence are live and PASS. The root New Bug whole-field visibility fix now removes both AI controls and generated labels locally; selective UI rollout and PM browser verification remain. Tester/Developer member-owned browser evidence is deferred. Evidence: `docs/pm/evidence/idts-115/`. |
| IDTS-105 | Consolidate mentor review requirements and enforce team acknowledgment | DonHV | In Progress. DonHV personally acknowledged merge SHA `3e78b495`; SangVN, DatDT and NhanT must still read the same briefing baseline and add their own Jira acknowledgments. |
| IDTS-106 | Retire Vietnamese Mentor Current artifacts and enforce English-only SAP490 output | DonHV | Done: PR #264 passed fresh QA Depth Gate and merged at `cd03aed`; archive tag/manifests preserve history, generators/validators are EN-only, 11 repo VI artifacts were retired, and exactly 13 Drive VI files were backed up and moved to Trash without emptying it. EN counterparts and Mentor Index were verified; Jira evidence comment `10882`. |
...
| IDTS-94 | Add review controls for Handoff Summary and Smart Assign explanation | SangVN | Done: PR #168 merged at `9e041da`; Knowledge Gate 3/3, focused checks, desktop/mobile browser regression and fresh QA Depth Gate passed. Jira closure evidence comment `10679`. |
| IDTS-95 | Confirm an accepted Similar Bugs suggestion as a DuplicateLink | SangVN | Runtime and focused QA pass on PR #172. DonHV approved an emergency deadline exception for the missing dedicated Knowledge Gate; the exception is not recorded as a learner PASS. |
| IDTS-97 | Add privacy-safe AI operational metrics | SangVN | Runtime, migration dry-run and full affected AI regression pass on PR #173. DonHV approved an emergency deadline exception for the missing metrics/privacy Knowledge Gate; the exception is not recorded as a learner PASS. |
| IDTS-96 | Add automated regression for AI review and apply actions | NhanT | Done via current-dev IDTS-68/69/91/93/95 suites, which cover persisted review, Handoff/Smart Assign no-mutation, classification apply and duplicate confirmation more deeply than stale PR #177. PR #177 closed without merge; Jira comment `10905`. |
| IDTS-98 | Build deterministic AI quality and safety evaluation dataset | NhanT | In Progress: stale PR #178 closed without merge because it was conflicting, included unrelated/unsafe files and used self-fulfilling heuristics. Rebuild only from current `dev` with actual feature calls and sanitized evidence; Jira comment `10906`. |
| IDTS-99 | Execute Shared QA live-provider acceptance for AI Phase 2 | NhanT | Done as superseded, not as an old-scope PASS. Render/OpenAI-era acceptance is replaced by SAP BTP/HANA/XSUAA work under IDTS-114/115; Jira comment `10907`. |
...
| IDTS-101 | Remediate SAP490 specifications using official templates | DonHV | Done: OfficeCLI 8/8, strict template/content validation, full sheet coverage, 105/105-page visual review, same-ID Drive update/readback 8/8, and Blueprint Drive footer pagination PASS. |
| IDTS-102 | Correct runtime trace, completeness, and formal layout across SAP490 specifications | DonHV | Done: PR #181 merged at `3d4e2d0`; eight EN/VI artifacts synchronized at existing Drive IDs; OfficeCLI 8/8, strict validator, 102/102-page visual review, secret/agent/depth/AI DevKit gates pass. Mentor approval remains external and Pending. |
| IDTS-103 | Formalize Functional and Technical Specification tables and Vietnamese parity | DonHV | Done: PR #183 merged at `5092035`; four approved workbooks were synchronized at their existing Drive IDs. OfficeCLI 4/4, strict specification/quality validation, 50/50-page local visual review, exact-byte Drive readback, and representative 9/9 plus 12/12-tab Drive previews pass. |
...
| ID | Cong viec | Thanh vien chinh | Ghi chu |
| --- | --- | --- | --- |
| IDTS-117 | Sua flow dang xuat va dang nhap lai SAP BTP | DonHV | Da xong: PR #254 merge tai `d733771`; deploy rieng AppRouter va hai vong Sign Out → dang nhap XSUAA → Fiori deu PASS. Evidence nam trong `docs/pm/evidence/idts-117/btp-rollout/`. |
| IDTS-113 | Chuyen IDTS Shared QA runtime va data len SAP BTP | DonHV | Dang lam: HANA/AppRouter/XSUAA PM, Job Scheduler + Brevo, History/Notifications, S3 adapter, AI fallback khong mutation, restart persistence va rollback platform-readiness da PASS tren SHA `3504931`. BTP user, role collection va HANA identity cua SangVN/DatDT/NhanT da khop; con evidence dang nhap va role matrix do tung member thuc hien. Render khong phai hot replica cua HANA. Con browser picker va tich hop Technical Specification EN qua IDTS-112. |
| IDTS-105 | Tong hop yeu cau mentor review va bat buoc team xac nhan da doc | DonHV | Dang lam. DonHV da tu xac nhan doc merge SHA `3e78b495`; SangVN, DatDT va NhanT van phai tu doc cung baseline va tu comment xac nhan tren Jira. |
...
| IDTS-94 | Them review control cho Handoff Summary va Smart Assign explanation | SangVN | Done: PR #168 merge tai `9e041da`; Knowledge Gate 3/3, focused check, browser desktop/mobile va fresh QA Depth Gate da pass. Jira closure evidence comment `10679`. |
| IDTS-95 | Xac nhan Similar Bugs suggestion da Accept thanh DuplicateLink | SangVN | Runtime va focused QA pass tren PR #172. DonHV duyet ngoai le khan theo deadline cho Knowledge Gate rieng con thieu; ngoai le khong duoc ghi thanh learner PASS. |
| IDTS-97 | Them operational metrics AI an toan ve privacy | SangVN | Runtime, migration dry-run va full AI regression bi anh huong pass tren PR #173. DonHV duyet ngoai le khan theo deadline cho Knowledge Gate metrics/privacy con thieu; ngoai le khong duoc ghi thanh learner PASS. |
| IDTS-96 | Them automated regression cho AI review va apply actions | NhanT | Done nho cac suite IDTS-68/69/91/93/95 tren current `dev`; coverage hien tai day du hon PR #177 ve persisted review, Handoff/Smart Assign no-mutation, classification apply va duplicate confirmation. PR #177 da dong khong merge; Jira `10905`. |
| IDTS-98 | Tao deterministic AI quality va safety evaluation dataset | NhanT | In Progress: PR #178 da dong khong merge vi conflict, co file khong lien quan/khong an toan va harness tu suy ra PASS. Neu giu scope thi phai viet lai tu current `dev`, goi feature logic that va sanitize evidence; Jira `10906`. |
| IDTS-99 | Chay Shared QA live-provider acceptance cho AI Phase 2 | NhanT | Done theo trang thai superseded, khong phai PASS scope cu. Baseline Render/OpenAI da duoc thay bang SAP BTP/HANA/XSUAA trong IDTS-114/115; Jira `10907`. |
...
| IDTS-101 | Sua bo SAP490 Specifications theo template chinh thuc | DonHV | Done: OfficeCLI 8/8, validator template/noi dung, day du sheet, visual review 105/105 trang, dong bo/readback Drive 8/8 va footer Blueprint tren Drive PASS. |
| IDTS-102 | Sua runtime trace, do day du va cach trinh bay formal cho bo SAP490 Specifications | DonHV | Done: PR #181 merge tai `3d4e2d0`; 8 artifact EN/VI da dong bo tai dung Drive ID; OfficeCLI 8/8, strict validator, visual 102/102 trang, secret/agent/depth/AI DevKit deu pass. Mentor phe duyet van la gate ben ngoai va dang Pending. |
| IDTS-103 | Chuan hoa bang trong Functional va Technical Specification va dong deu ban VI | DonHV | Done: PR #183 merge tai `5092035`; bon workbook da duoc dong bo tai dung Drive ID. OfficeCLI 4/4, validator nghiem ngat, visual local 50/50 trang, Drive readback khop byte va preview dai dien 9/9 cung 12/12 tab deu pass. |
...
| --- | --- | --- | --- |
| IDTS-107 | Rebuild Technical Design, database dictionary, and persistence evidence | Gate 2 approved by DonHV at content head `4cca4c0bc575469810c881b1757e6eb3f519437c`; 48/578 production build and live HANA evidence agree. | Merge PR #265 normally and hand the approved package to IDTS-112; runtime helper-table finding remains in IDTS-118. |
...
| IDTS-111 | Execute SAP490 UAT and publish case-level image evidence | In Progress — catalog approved and frozen SAP BTP execution baseline published; all 90 cases remain PREPARED. | NhanT, SangVN, DatDT and DonHV execute only assigned cases with member-owned identities and case-specific evidence; DonHV reviews results before workbook generation. |
...
| --- | --- | --- | --- |
| IDTS-107 | Lam lai Technical Design, database dictionary va persistence evidence | DonHV da duyet Gate 2 tai content head `4cca4c0bc575469810c881b1757e6eb3f519437c`; production build va HANA live khop 48/578. | Merge PR #265 binh thuong va handoff package da duyet sang IDTS-112; finding helper table tiep tuc o IDTS-118. |
...
| IDTS-111 | Chay UAT SAP490 va cong bo hinh anh theo tung case | Dang thuc hien — catalog da duyet va baseline BTP da freeze; tat ca 90 case van PREPARED. | NhanT, SangVN, DatDT va DonHV chay dung case duoc giao bang tai khoan rieng, thu evidence rieng; DonHV review truoc khi sinh workbook. |
