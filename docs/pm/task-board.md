...
| IDTS-94 | Add review controls for Handoff Summary and Smart Assign explanation | SangVN | Implementation and dedicated local evidence are complete on the stacked branch. Focused/backend suites, UI5 build/lint, and both contextual browser suites pass; review persistence does not mutate Bug workflow or assignment. Await stacked PR review, IDTS-91 foundation merge, deferred Knowledge Gate, and Jira closure. |
