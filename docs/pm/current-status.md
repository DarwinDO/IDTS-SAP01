...
- Ordinary `BugService.Bugs` reads are not newly attributed to Gate 6.3 and were not changed. The branch remains source-only; the new exact-head review, fresh full matrix, push/PR readback and stop-at-Draft boundary remain pending. No Ready, merge, deploy, data/provider/user/role/email/Jira/Drive mutation, Gate 6.4 or cleanup.
