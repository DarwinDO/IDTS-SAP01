...
| My Notifications planning | DonHV approved a caller-only federated inbox plus prompt post-commit email for important Bug/access events. Job Scheduler is recovery and scheduled SLA/Overdue/digest/retention work, not the normal one-hour status-email path. Design is documented; N1-N6 implementation and every migration/rollout boundary remain unstarted and separately approved. |
