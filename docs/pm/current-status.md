...
| Active DatDT implementation | IDTS-91/92/93 were verified and merged through PR #167. Explicit suggestion review actions, persisted review controls, and the separate Tester/PM classification-apply action are now in `dev`. Jira closure remains a tracking follow-up. |
| Active SangVN implementation | IDTS-94 was verified and merged through PR #168 with Ownership Knowledge Gate 3/3 (100%). IDTS-95 is being integrated on top of that baseline; its runtime/evidence exist, but merge remains blocked until SangVN completes the separate duplicate-confirmation Knowledge Gate. |
