...
- IDTS-42 is merged into `dev` through GitHub PR #38. The repository now has a mandatory QA Depth Gate baseline: PR evidence template, `qa-depth-gate` workflow, PR-body validator, reusable browser harness, secret scan, and documented falsification-first QA rules. Branch protection still needs to require this workflow after it is available on `dev`.
...
- IDTS-42 da merge vao `dev` qua GitHub PR #38. Repo hien co baseline QA Depth Gate bat buoc: PR evidence template, workflow `qa-depth-gate`, validator PR body, browser harness dung lai, secret scan va rule QA falsification-first. Sau khi workflow co tren `dev`, can bat branch protection yeu cau workflow nay pass.
