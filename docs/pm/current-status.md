...
## 2026-09-02 Ownership learning becomes opt-in

- **English:** DonHV retired the mandatory task-start Ownership Knowledge Gate and its PR/Jira blocking behavior. Learning Recap, mentor mode, guided debugging, teach-back and optional quizzes now run only when the user explicitly requests learning. QA Depth still requires technical evidence, role/authorization, persistence/reload and UI/UX reasoning. The separate SAP490 Mentor-Review Briefing Gate remains unchanged. Historical Knowledge Gate evidence is preserved as history, not current enforcement.
- **Tiếng Việt:** DonHV đã bỏ Ownership Knowledge Gate bắt buộc đầu task và cơ chế chặn PR/Jira của nó. Learning Recap, mentor mode, debug có hướng dẫn, teach-back và quiz tùy chọn nay chỉ chạy khi user chủ động yêu cầu học. QA Depth vẫn yêu cầu evidence kỹ thuật, role/authorization, persistence/reload và UI/UX. SAP490 Mentor-Review Briefing Gate riêng vẫn giữ nguyên. Evidence Knowledge Gate cũ được giữ làm lịch sử, không còn enforcement hiện tại.
...
| IDTS-111 / IDTS-127 UAT handoff | Forward catalog: 77 PREPARED cases. Retained NhanT package: 44 manifests at 27 MEETS / 7 DOES_NOT_MEET / 10 BLOCKED, with 82 hash-verified evidence references (72 unique hashes). Cases 39–41 attachment upload/download/delete are final-approved from one exact 118-byte fixture chain; AI-005/009 and partial UX-002 remain explicit gaps. Final UAT workbook/Drive remain unchanged. |
...
| Optional ownership learning | Mandatory Knowledge Gate enforcement is retired by DEC-070. Ownership maps, learning notes, Debug Labs, mentor mode and the `learning:gate` selector remain available only when a user explicitly requests learning; they do not block tasks, PRs or Jira. |
...
- Independent SAP490 recheck findings were closed on 2026-07-23 at the existing Drive IDs: Test Report EN/VI use the exact v0.3 header with no broken defined names, Diagram 07 no longer shows Request More Information after retest, and Blueprint EN/VI are 20 pages each. A second active verification confirmed the artifact verdict remained CONDITIONALLY READY at that time: the reproducible Office composition was 27 local artifacts + one Drive-only XLSX + three native Sheet exports = 31/31 PASS/zero issues; the diagram manifest was 21/21, native Slides were 46/46/46 with zero legacy, Workshop metadata was unchanged, and runtime diff was empty. Its former Ownership Knowledge Gate reopening requirement is superseded by DEC-070; current SAP490 readiness is governed by the separate Mentor-Review Briefing Gate and technical/submission evidence. Contextual `TBD` approval dates and revision-history references to removed placeholders are not unresolved defects.
