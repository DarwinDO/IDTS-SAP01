...
| Active audit refactor | `IDTS-89`: source implementation ready; database narrow UPSERT complete with 11/11 exact codes and preserved business/history/user/legacy rows; runtime deploy blocked by the separate Knowledge Gate and unsafe broad pre-deploy behavior. PR #163 remains Draft/unmerged. A runtime-only release plan is proposed but not approved or executed. |
