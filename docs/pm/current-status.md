...
| Active audit refactor | `IDTS-89` implementation, local review, and Shared QA ActionTypes UPSERT/readback are complete. PostgreSQL now has 11/11 exact codes; Bugs/History/Users and 11 legacy codes are preserved. Exact workflow smoke remains blocked because live Render commit `07be39e` predates `origin/dev` `7cb2d54` and the IDTS-89 branch; do not substitute the broader `cds-deploy`. |
