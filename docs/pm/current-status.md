...
| Active implementation note | DatDT completed implementation and focused verification for IDTS-91/92/93 on `feature/idts-91-93-implementation-datdt`: explicit suggestion review actions, persisted review controls in the existing dialogs, and a separate Tester/PM validated classification-apply action. PR #167 is open against `dev`; focused/backend/UI/browser evidence is green and the GitHub QA depth gate is running. Jira review/closure and merge remain pending. |
| Active AI follow-up | SangVN is implementing IDTS-95 under supervision on `feature/idts-95-confirm-duplicate-suggestion-sangvn`, based on dependency PR #167. Focused confirmation QA passes `31/0`; related IDTS-66/91/93 and security regressions remain green. The Ownership Knowledge Gate is deferred, so no merge or Jira Done is allowed until PASS. |
