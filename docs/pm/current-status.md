...
| Active audit refactor | `IDTS-89`: Knowledge Gate 90% PASS and required gate PASS; database narrow UPSERT complete. Runtime release guard is active: Shared QA auto-deploy is off and pre-deploy is `true`, with build/start/branch/health unchanged. PR #163 remains Draft until this checkpoint commit passes the required gate again; runtime not deployed yet. |
