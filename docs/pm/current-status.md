...
| My Notifications planning | DonHV approved a caller-only federated inbox plus prompt post-commit email for important Bug/access events. Design and the N1-N6 implementation plan are documented. Source remains unstarted; N1 is dependency-blocked until Gate 6.5 merges into `origin/dev`. Job Scheduler remains recovery/SLA/Overdue/digest/retention, not the normal one-hour prompt-email path. |
