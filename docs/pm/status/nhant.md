...
| 2026-07-25 | IDTS-99 | Executed Shared QA live-provider acceptance tests using Playwright automation. Script `test-idts99-shared-qa-live.js` successfully logged into Live Render QA, checked Fiori AI action dialogs. | All AI review actions leave workflow and ownership unchanged | **Testing issue:** Tester role properly hidden from AI actions (expected); PM role timed out initially due to lazy loading and vi-VN OS language defaults. | Fixed by adding explicit scroll commands and forcing en-US Playwright locale. | `npm run qa:idts99:shared-qa-live` -> 10/10 PASS (Evidence saved) | PR to merge `qa/idts-99-shared-qa-acceptance-nhant` into `dev` |
