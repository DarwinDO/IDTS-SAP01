...
| 2026-07-25 | IDTS-96 | Chạy và verify các file test programmatic (IDTS-91, 93, 95) do DatDT viết thay vì viết lại kịch bản mới. Ghi nhận log và lưu evidence. | Hoàn thành verify toàn bộ các luồng Accept, Reject, Ignore, Handoff, và Apply Classification. | Không có bug/error, test backend đã chạy hoàn chỉnh do DatDT viết trước đó. | Đã verify và ghi nhận | `npm run qa:idts91:programmatic`, `qa:idts93`, `qa:idts95` | Cập nhật Jira IDTS-96 thành DONE |
| 2026-07-25 | IDTS-98 | Xây dựng dataset test các tình huống tiếng Việt, Tiếng Anh, prompt injection, sparse context, và sửa script đánh giá sang OData Action calls (`tx.send`). Fix lỗi test logic phần fallback. | 8 test cases chạy ổn định, tạo thành công report. | **Test-harness issue**: BugManagementService read deadlock; Test framework đánh giá sai fallback an toàn thành unsafe; Thiếu DeveloperResponsibilities trong seed data làm Smart Assign mock fail. | Đã rollback transaction đúng lúc để fix deadlock. Update điều kiện `actualAbstain` nhận diện `LOW_CONFIDENCE` fallback đúng chuẩn. Thêm seed data. | `npm run qa:idts98:evaluation` PASS 100% (8 cases). Report được tạo tự động. | Tạo PR IDTS-98, đợi DonHV review |
