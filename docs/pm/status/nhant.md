...
- IDTS-12: Demo script created, final smoke test re-confirmed (20/21 PASS), pending team evidence and commit.

Vietnamese: IDTS-12: Demo script đã tạo, final smoke test xác nhận lại (20/21 PASS), chờ evidence từ team và commit.
...
| 2026-06-17 | IDTS-12 | Created `docs/demo/sprint02-demo-script.md` with 10-scene demo flow, backend verification commands, known issues list, and deliverables summary; re-ran final smoke test; ran CDS compile check | Demo script created; smoke test re-confirmed 20/21 PASS (same as 06-13); CDS compile EDMX OK; 6 known issues documented honestly | None new | All items completed | `node scripts/qa/test-idts6-programmatic.js` → 20 PASS 1 SKIP 0 FAIL; `cds compile srv app/bug-management-ui --to edmx` → EDMX OK | DonHV/DatDT/SangVN to add their evidence before mentor meeting; commit and close IDTS-12 |
...
| 2026-06-17 | IDTS-12 | Tạo `docs/demo/sprint02-demo-script.md` gồm 10 scene demo, lệnh verify backend, danh sách known issues, và tổng hợp deliverables; chạy lại final smoke test; chạy CDS compile check | Demo script đã tạo; smoke test xác nhận lại 20/21 PASS (như 06-13); CDS compile EDMX OK; 6 known issues ghi nhận trung thực | Không có mới | Tất cả đã hoàn thành | `node scripts/qa/test-idts6-programmatic.js` → 20 PASS 1 SKIP 0 FAIL; `cds compile srv app/bug-management-ui --to edmx` → EDMX OK | DonHV/DatDT/SangVN bổ sung evidence trước buổi mentor; commit và đóng IDTS-12 |
