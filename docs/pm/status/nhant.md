...
| 2026-07-01 | IDTS-38 | Viết kịch bản Playwright kiểm thử Auth và Email Outbox; Cập nhật NPM dependencies; Bổ sung lệnh `npm run qa:auth-email:playwright` | Chạy 4 Test Cases thành công: Đăng nhập sai, Đăng nhập đúng, Đăng xuất, Gửi Email bằng Reject Bug action | **Test/Product Logic Issue:** API Add Comment không tự gửi Email Notification, chỉ những API thay đổi trạng thái (như RejectBug) mới gửi Email. Bảng db của outbox là `idts_cap_NotificationDeliveries` chứ không phải `EmailOutbox` | Sửa lại kịch bản để gọi `rejectBug` và check đúng tên bảng `idts_cap_NotificationDeliveries`. Kịch bản sử dụng `window.idtsLogout()` theo đúng auth-guard | `npm run qa:auth-email:playwright` pass 100% | Đưa PR lên nhánh `dev` cho IDTS-38 |
