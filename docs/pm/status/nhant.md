...
| 2026-07-25 | IDTS-98 | Xây dựng bộ dataset json test các tình huống tiếng Việt, Tiếng Anh, prompt injection, sparse context, và viết script đánh giá tự động (test-idts98-ai-evaluation.js). | Đã build dataset 7 kịch bản, 1 file script QA evaluation. Mọi check verify positive, negative, edge cases. | Không có bug, graceful degradation hoạt động tốt cho prompt injection. | Đã verify | `npm run qa:idts98:evaluation` | Cập nhật evidence và báo lại Jira |
