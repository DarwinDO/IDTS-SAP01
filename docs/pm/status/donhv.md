...
| 2026-06-15 | Audit Jira và pull request | Query project Jira `IDTS` qua Atlassian Rovo, lấy danh sách branch/PR GitHub và đồng bộ phần PM handover | Xác nhận 6 Done, 4 In Progress, 2 To Do; có 2 PR đang mở và 0 PR đã merge | PM docs bị cũ; GitHub CLI `gh` chưa có trên PATH | Đã cập nhật PM docs; dùng GitHub REST API thay thế thành công. Thiếu `gh` không chặn công việc | Atlassian Rovo `_searchjiraissuesusingjql`; `git fetch --prune origin`; GitHub REST API; `git diff --check` | Review PR #1 và PR #2; tạo PR cho branch DonHV/DatDT đã hoàn thành khi phù hợp |
| 2026-06-15 | Audit branch UI DatDT trước merge | Kiểm tra toàn bộ local/remote branch, author commit, UI diff, lịch sử PR và mô phỏng kết hợp `Remake_UI` của DatDT với branch PR #2 của SangVN | Xác nhận `Remake_UI` là remote UI branch riêng duy nhất của DatDT và có một commit mới trên `dev` | `Remake_UI` chưa có PR và conflict với PR #2 tại `app/bug-management-ui/annotations.cds`; branch còn giảm Create required fields và đổi CreationDialog sang NewPage | Rủi ro merge còn mở; không merge nguyên branch. Chỉ review/cherry-pick phần đã được chấp nhận sau khi review PR #2 | `git for-each-ref`; GitHub branches/PR API; `git log --all`; `git diff`; `git merge-tree --write-tree` | Review PR #2 trước, sau đó so sánh và tích hợp chọn lọc thay đổi của DatDT |
