...
- Rerun 40 exact LOCAL cases and 135 corrected local-primary cases on Node.js 22.23.1.
- Publish current truth: 172 candidate PASS, 1 FAIL and 15 BLOCKED; keep the approved catalog `NOT_RUN` until DonHV accepts results.
- Remove 278 duplicate SVG evidence sources and retain 188 manifests, 278 PNGs and structured JSON evidence.

## Positive Evidence

- Exact LOCAL runner: 40 cases — 37 PASS / 1 FAIL / 2 UI BLOCKED.
...
- `UT-AUTH-004` remains FAIL: HTTP 400 and unchanged `AuthSessions` are correct, but the public response exposes CAP type-validation internals (`ASSERT_DATA_TYPE`, `String(255)`, rejected value).
...
- `UT-ATT-007` and `UT-ATT-008` retain BLOCKED because static MIME/10 MB guards are not UI runtime proof.
...
- `UT-AUTH-004` records unchanged session count despite its public-error sanitization FAIL.
...
- The deployed SAP page is currently blank and logs `Unexpected token '<'` for `auth-guard.js` and `bootstrap-ui5.js`; therefore both UI component cases remain BLOCKED.

## Ponytail Simplicity

- Reuse existing domain suites and add only one small exact HTTP harness plus one aggregate mapping runner.
- No new project dependency, production abstraction, application behavior or deployment configuration was added.
- Portable Node.js 22 was used without rebuilding native dependencies or changing the lockfile.
...
- One open product/security-boundary finding: `UT-AUTH-004`.
- Two UI component cases require a healthy deployed SAP/browser surface.
