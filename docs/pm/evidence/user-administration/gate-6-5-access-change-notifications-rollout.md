...
- A later pre-PR readiness check found CAP, AppRouter, and broker stopped together by the SAP BTP trial scheduled auto-stop at `00:02:44`; all existing routes remained intact and prior-day audit events showed the same daily pattern. The existing check-first prepare script started CAP/AppRouter without a HANA update, restage, or deployment; broker was started once. Fresh readback returned CAP/AppRouter `1/1`, broker `STARTED` with its web instance running, health/readiness/web `200`, anonymous protected API `401`, and `DEMO READY`.
