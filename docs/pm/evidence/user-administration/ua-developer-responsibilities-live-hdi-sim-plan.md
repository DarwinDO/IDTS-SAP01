...
Date: 2026-08-19
Owner: DonHV
Status: `EXACT R6 PLAN / STANDING DONHV GO / STOP ON DRIFT`

## Frozen local artifact

- Archive: `mta_archives/idts-ua-developer-hdi-sim-r6.zip`.
- SHA-256: `4014eec047f1e971fb6c52fb2d02c60ab7992fd696b93c9c8bbbb2dedc9ca6aa`.
- Size: `9,316` bytes.
- File entries: exactly `16`.
...
- Temporary app: `idts-ua-developer-hdi-sim-20260819-r6`.
- Temporary task: `idts-ua-developer-hdi-sim-20260819-run-r6`.
- Routes: `0`.
- Instances started: `0`.
- Memory: `256M`.
- Disk: `512M`.
...
6. Archive SHA-256 and all 16 entry hashes equal the approved artifact.
...
```powershell
cf push idts-ua-developer-hdi-sim-20260819-r6 `
  --no-manifest `
  --no-start `
  --no-route `
  -p mta_archives/idts-ua-developer-hdi-sim-r6.zip `
  -m 256M `
  -k 512M `
  -b nodejs_buildpack
```
...
```powershell
cf bind-service idts-ua-developer-hdi-sim-20260819-r6 idts-sap01-db
```
...
```powershell
cf stage-package idts-ua-developer-hdi-sim-20260819-r6
```
...
```powershell
cf set-droplet idts-ua-developer-hdi-sim-20260819-r6 <IN_MEMORY_OWN_STAGED_DROPLET_GUID>
```
...
```powershell
cf run-task idts-ua-developer-hdi-sim-20260819-r6 `
  --command "node scripts/btp/ua-developer-hdi-simulate-command.js" `
  -m 256M `
  -k 512M `
  --name idts-ua-developer-hdi-sim-20260819-run-r6 `
  --wait
```
...
```powershell
cf unbind-service idts-ua-developer-hdi-sim-20260819-r6 idts-sap01-db
cf delete idts-ua-developer-hdi-sim-20260819-r6 -f
```
