...
- Source implementation head before this evidence commit: `363b6a04f103dd0c60c7882b40a9c912ac74d5df`.
- Scope: named BugService model, read-only Developers → Workload overview, bounded assigned non-Closed Bug details, exact same-origin Bug Object Page links, focused contracts, bilingual mirrors, PM evidence and one Draft PR handoff.
- Explicitly out of scope: `db/`, `srv/`, schema/HANA/HDI, dependency/lockfile, assignment/status mutation, platform/provider/user/role/data/email mutation, deployment, merge, Ready transition, runtime rollout, Gate 6.4 and worktree cleanup.
...
| `npm run qa:developer-workload:programmatic` | PASS: `39 PASS / 0 FAIL`, proving the reused BugService aggregate semantics. |
...
- The Workload UI does not authorize any operation. User Administration remains behind its existing PM + `UserAdmin` route/CAP authorization; direct role enforcement remains server-owned.
...
6. Verify Tester and Developer direct access remains Forbidden and that no workload action changes Bug assignment/status/history/notification data.
...
| Repository | Only the Gate 6.3 app source, QA contract/package alias and required documentation/PM files are changed. |
...
Updated matching bilingual mirrors for `manifest.json`, `Main.controller.js`, `Main.view.xml`, `formatter.js`, `i18n.properties`, `i18n_en.properties`, `i18n_vi.properties`, and the new `DeveloperWorkloadDetails.fragment.xml`. Each mirror explains the IDTS flow, source anchors, impact, linked contracts and safe-editing rules.
...
Các contract TDD, suite User Administration/Active Users/User Access, workload backend `39/0`, secret scan, agent rules, QA-depth, CAP EDMX/HANA compile, UI5 MCP lint/manifest validation, UI lint/build và diff guard đều đã có evidence mới. Đây chỉ là source gate; chưa có runtime/manual acceptance, deploy, mutation dữ liệu/provider/user/role/email, merge, Ready hoặc Gate 6.4.
