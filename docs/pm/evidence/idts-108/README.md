...
- CAP MCP re-confirmed `BugService` at `srv/service.cds:5`, its OData V4 path and
  ...
  and the attachment download disposition annotation in `db/schema.cds`; it does
  not change the public CAP service contract.
...
| 2 | Signed-in shell and profile | App bootstrap and profile button | User name/role, dashboard navigation, sign out | Tester, Developer, PM | Sign out clears browser session and invokes Auth logout | `Component.js`; `auth-guard.js`; `ext/login/ProfileShell.js` | UI capture PASS / NETWORK KNOWN LIMITATION — Tester, Developer and PM role shells plus bidirectional account switching are captured; direct logout/auth readback remains outside this package |
| 3 | Role Dashboard | `dashboard.html` or Open Dashboard | Role-specific KPI tiles, focus list, developer workload, PM AI activity | Tester, Developer, PM; AI activity PM only | Focus item opens Bug Object Page; back opens List Report | `dashboard-page.js`; `BugService.DeveloperWorkloads`; `readAiOperationalMetrics` | UI capture PASS — Developer, Tester and PM dashboards are captured; screenshots 49–50 additionally prove PM workload/attention monitoring and the authorized PM AI Activity aggregate dialog. Network and non-PM denial remain separate evidence gaps |
...
| 5 | Create Bug | Fiori Create action, new draft Object Page | General, supporting, classification, reproduction, planning, Smart Assign, generated attachment facet | Tester, PM | Save activates Bug and its draft composition; cancel discards the draft; assignee empty produces Pending Assignment | `BugListActions.createBug`; `object-page.cds`; `drafts.js`; `bug-write.js` | UI capture PASS — screenshots 29–33 prove empty/validation/Pending Assignment create and reload; screenshots 35–38 prove the Assigned create and reload variant with SangVN ownership. The current generated attachment flow and its Network/database effects remain separate acceptance evidence |
...
| 51 | Attachments | Select / upload file | Generated `attachments/@UI.LineItem` facet | File plus attachment draft row | Tester, Developer, PM subject to backend access | Standard Fiori/CAP attachment validation applies; change remains in the parent draft until Save |
| 52 | Attachments | Download | Generated attachment content operation | Attachment row | Authorized Bug viewer | Stream failure shows standard safe handling; no metadata mutation |
| 53 | Attachments | Delete | Generated attachment row delete plus parent Save/Discard | Attachment row plus confirmation/draft decision | Authorized participant | Discard performs no active mutation; failed Save keeps the active attachment state unchanged |
...
| 29 | Read role dashboard | `dashboard-page.js::loadDashboard` | Reads Bugs and DeveloperWorkloads | CAP read models and auth | Builds role-specific tiles/focus rows without mutation | UI capture PASS / NETWORK KNOWN LIMITATION — Tester, Developer and PM dashboards are captured; sanitized OData requests are not |
...
| Comments and attachments | `app/bug-management-ui/webapp/ext/sections/BugCollaboration.js::onAddComment`; `app/bug-management-ui/webapp/ext/fragment/CommentsSection.fragment.xml`; `app/bug-management-ui/annotations/object-page.cds`; `db/schema.cds::Bugs.attachments` (`@Core.ContentDisposition.Type: 'attachment'`); `srv/bug-service/content.js` | UI5 OData V4 bound comment action, independent comments refresh, generated Fiori attachment facet and browser-download disposition trace confirmed on the current baseline |
...
| `screenshots/06-developer-history.png` | Existing lifecycle/edit events that are append-only by service behavior, plus Show Details controls | No paging or expanded-detail proof |
...
- [x] SangVN personally acknowledges briefing SHA `3e78b495cb8feb56188cc446b827d47e040e1b98` in Jira comments `10876` and `10877`, including assigned Developer-role UAT ownership, and personally updated the matching repository register row.
- [x] SangVN captured and sanitized the currently available scoped UI/network evidence; every unexecuted result remains an explicit known limitation or deferred owner action.
...
1. SangVN briefing acknowledgment is `READ` for SHA `3e78b495` in Jira comments
   `10876` and `10877`, and the matching repository register row is updated. Candidate
   approval remains separate and Pending for the exact refreshed PR head.
