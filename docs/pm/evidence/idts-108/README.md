...
- UI5 MCP returned the project identity and current async-module/data-binding
  ...
  fragments, controllers and build validation remain the evidence baseline for that
  layer.
- UI5 MCP manifest schema validation passed. Its linter found pre-existing manifest
  version and integration-test bootstrap debt; this is tracked separately by linked
  Jira task `IDTS-121` and is not modified or hidden by PR #256.
...
| ---: | --- | --- | --- | --- | --- | --- | --- |
| 1 | Sign In | `login.html` | SAPUI5 login form, email, password, safe message strip, Sign In | Anonymous | Success opens `index.html`; failure remains on form | `login-page.js`; `srv/auth.cds`; `srv/auth.js` | PARTIAL / KNOWN LIMITATION — sanitized identity/login validation and successful role shells are captured; a sanitized authentication Network exchange is not available |
| 2 | Signed-in shell and profile | App bootstrap and profile button | User name/role, dashboard navigation, sign out | Tester, Developer, PM | Sign out clears browser session and invokes Auth logout | `Component.js`; `auth-guard.js`; `ext/login/ProfileShell.js` | UI PASS / NETWORK KNOWN LIMITATION — Tester, Developer and PM role shells plus bidirectional account switching are captured; direct logout/auth readback remains outside this package |
| 3 | Role Dashboard | `dashboard.html` or Open Dashboard | Role-specific KPI tiles, focus list, developer workload, PM AI activity | Tester, Developer, PM; AI activity PM only | Focus item opens Bug Object Page; back opens List Report | `dashboard-page.js`; `BugService.DeveloperWorkloads`; `readAiOperationalMetrics` | UI PASS — Developer, Tester and PM dashboards are captured; screenshots 49–50 additionally prove PM workload/attention monitoring and the authorized PM AI Activity aggregate dialog. Network and non-PM denial remain separate evidence gaps |
| 4 | Bug List Report | `index.html`, route `BugsList` | Filter bar, six view tabs, grid table, Dashboard and Create actions | All authenticated; Create only Tester/PM | Row opens Object Page; Create opens new draft page | `manifest.json`; `annotations/list-report.cds`; `annotations/pm-monitoring.cds`; `BugListActions.js` | PARTIAL — Developer, Tester and PM role variants are captured; screenshot 48 shows the PM monitoring filters, six tabs, rows, Dashboard and Create actions. Filter execution and Network requests remain open |
...
| 6 | Bug Object Page | Route `Bugs({key})` | Header, summary, classification/planning, reproduction, Smart Assign, comments, attachments, history, notifications, lifecycle toolbar | Tester, Developer, PM with backend ownership checks | Edit/save, actions and child sections refresh through OData side effects | `manifest.json`; `object-page.cds`; `actions.cds`; `service.cds` | PARTIAL — Developer, Tester and PM display/action variants are captured; screenshot 51 proves PM can read BUG-0024 and sees coordinator actions. Successful edit/save and action side effects remain open |
...
| 8 | Classification review | Open Classification Suggestions after classification fields | Suggested values, confidence/reason, Accept/Reject/Ignore, Apply | Tester/PM apply; backend review authorization | Apply changes classification only for accepted current suggestion | `ClassificationReviewField.fragment.xml`; `ClassificationReview.js`; `classification-suggestion.js`; `classification-apply.js` | PARTIAL / KNOWN LIMITATION — screenshot 42 proves the advisory dialog and review/apply controls; no decision, Apply, stale-conflict, persistence or Network evidence was submitted |
| 9 | Assignment and Smart Assign | Smart Assignment custom section and developer picker | Assignee input, candidate search/list, workload/availability, explanation, review buttons, Assign | Tester/PM assign; explanation remains advisory | Assign invokes bound action; empty selection is not auto-assigned | `SmartAssignmentSection.fragment.xml`; `SmartAssignDeveloper.js`; `AssignableDevelopers`; `assignment-explanation.js` | PARTIAL / KNOWN LIMITATION — screenshot 47 proves filtered candidates, capability, availability, explanation, confidence and manual controls; review/assign/invalid-assignee mutation and Network evidence remain open |
| 10 | Move to Pending Assignment | Object Page action dialog | Optional Reason | Tester/PM; visibility from `canMoveToPending` | Clears assignee, sets Pending Assignment and next processor | `actions.cds`; `service.js`; `actions.js` | KNOWN LIMITATION — Pending Assignment create/reload is proven, but the dedicated move action request/result/history was not executed |
| 11 | Mark In Review | Object Page action dialog | Optional Developer Note | Assigned Developer or Tester/PM | Status becomes In Review | Same sources; `markInReview` | KNOWN LIMITATION — no request/result/history evidence was executed for this action |
| 12 | Request More Information | Object Page action dialog | Required Reason | Assigned Developer or Tester/PM | Status becomes Need More Information; Tester/PM becomes next actor | Same sources; `requestMoreInformation` | PARTIAL / KNOWN LIMITATION — screenshots 13–14 prove the control/dialog and required Reason field; validation failure, success, Network and persistence are open |
| 13 | Resubmit to Developer | Object Page action dialog | Required Update Summary | Tester/PM | Status returns to Assigned; comment/history/notification are written | `actions.cds`; `actions.js::resubmitToDeveloper` | KNOWN LIMITATION — failure/success request, comment, history and notification evidence was not executed |
| 14 | Reject Bug | Object Page action dialog | Required Rejection Reason | Assigned Developer or Tester/PM | Status becomes Rejected with follow-up owner | `actions.cds`; `service.js`; `actions.js::transitionBug` | PARTIAL / KNOWN LIMITATION — screenshot 15 proves the dialog and required reason; validation failure, success, Network and persistence are open |
| 15 | Start Progress | Object Page action dialog | Optional Developer Note | Assigned Developer or Tester/PM | Status becomes In Progress | Same sources; `startProgress` | KNOWN LIMITATION — no request/result/history evidence was executed in this package; assigned UAT execution is tracked separately under IDTS-111 |
| 16 | Resolve Bug | Object Page action dialog | Required Developer Note | Assigned Developer or Tester/PM | Status becomes Resolved; Tester/PM follow-up | Same sources; `resolveBug` | PARTIAL / KNOWN LIMITATION — screenshot 16 proves the dialog and required note; validation failure, success, Network and persistence are open |
...
| 20 | Comments | Object Page Comments section | Comment input, Post, comment list | Tester, Developer, PM; active Bug only | Add Comment refreshes Bug/comments/history | `CommentsSection.fragment.xml`; `BugCollaboration.onAddComment`; `BugService.addComment` | PARTIAL / KNOWN LIMITATION — enabled/disabled states and two safe failures are captured; no successful post, sanitized Network response or reload persistence exists in this package |
| 21 | Evidence / Attachments | Object Page Attachments section | File picker, pending list, uploaded table, Download, Delete | Tester, Developer, PM subject to backend access | New-draft files queue in browser; active upload persists metadata/binary | `AttachmentsSection.fragment.xml`; `BugCollaboration.js`; `content.js`; attachment model annotations | PARTIAL / KNOWN LIMITATION — safe file selection and failed upload UI are captured; successful metadata/binary persistence, download/hash, delete and reload evidence remain DonHV/runtime follow-up |
| 22 | History Timeline | Object Page History section | Handoff action, growing event list, nested change table, reason/status | Authenticated Bug viewer | Show More pages immutable events; no mutation | `HistoryTimeline.fragment.xml`; `history-read-models.js`; `history.js` | PARTIAL / KNOWN LIMITATION — existing immutable events and detail controls are captured; next-page request and expanded nested changes are not |
...
| 25 | AI Activity metrics | PM Dashboard action | Feature totals, success/failure/timeout/unavailable, review counts, latency | PM only | Read-only aggregate; close returns to dashboard | `dashboard-page.js::openAiActivity`; `readAiOperationalMetrics`; `metrics.js` | PARTIAL — screenshot 50 proves the authorized PM aggregate with four capabilities, request/success/failure counts, latency and review decisions. Non-PM denial and Network response remain open |
| 26 | Operational evidence surfaces | External tools only when proving integration | SAP BTP Cockpit/Cloud Foundry, SAP HANA Database Explorer, Job Scheduling Service, Vercel AI Gateway, S3 and Brevo/inbox; not IDTS runtime screens. Render/PostgreSQL are rollback/reference only | DonHV / authorized operator | Evidence is linked to the matching runtime flow | DonHV-provided database/provider/integration evidence | DEFERRED OWNER ACTION — known external limitation for the SangVN package; DonHV supplies or dispositions it during IDTS-112 integration |
...
| ---: | --- | --- | --- | --- | --- | --- |
| 1 | Sign in | `login-page.js::submitLogin` | `POST /odata/v4/auth/login` | `srv/auth.js::login`; active user and password hash checks | Reads Users, creates AuthSessions, returns raw token once | PARTIAL / KNOWN LIMITATION — sanitized UI validation and successful BTP role shells exist; no sanitized auth request/response capture |
| 2 | Create Bug draft | List Create -> Fiori new page | OData V4 draft `NEW`, `PATCH`, `SAVE` | `prepareDraftNew`, `prepareDraftPatch`, `handleDraftSave`, `prepareBugWrite` | Activates Bug as Assigned or Pending Assignment; history/notification follow | PARTIAL / KNOWN LIMITATION — both active results and reload persistence are captured; draft NEW/PATCH/SAVE requests and database side effects are not |
| 3 | Classification value help | Fiori generated inputs | Reads SAPModules, ApplicationComponents, ValidDefectCategories | CAP projections and active-pair validation | Returns active values; invalid pair blocks save | PARTIAL / KNOWN LIMITATION — successful controlled value-help selections are reflected in the created Bugs; dependent read requests and invalid-pair rejection are not captured |
| 4 | Similar Bugs suggestion | `DuplicateReview.openDialog` | Unbound `suggestSimilarBugs` | `duplicate-detection.js`; safe input and source checks | Returns ranked candidates; optional safe AiSuggestions audit; no DuplicateLink | PARTIAL / KNOWN LIMITATION — screenshot 34 proves normal ranked output; fallback, Network and audit persistence are open |
| 5 | Confirm duplicate | Duplicate review Confirm | Unbound `confirmDuplicateSuggestion` | `duplicate-confirmation.js`; role, accepted state, membership, duplicate checks | Inserts one DuplicateLink or rejects atomically | KNOWN LIMITATION — accepted/conflict mutation was intentionally not executed in this package |
| 6 | Classification suggestion | `ClassificationReview.openDialog` | Unbound `suggestClassification` | `classification-suggestion.js`; context/source validation | Returns reviewable candidate rows; no automatic Bug mutation | PARTIAL / KNOWN LIMITATION — screenshot 42 proves normal advisory output; unavailable/fallback and Network evidence are open |
| 7 | Apply classification | Classification Apply | Unbound `applyClassificationSuggestion` | `classification-apply.js`; Tester/PM, accepted/current, stale-state checks | Updates validated classification and audit/history in controlled transaction | KNOWN LIMITATION — no Apply/stale-conflict mutation, before/after, history or Network evidence was executed |
| 8 | Explain Smart Assign | `SmartAssignDeveloper` | Unbound `explainSmartAssignment` | `assignment-explanation.js`; classification and source checks | Returns explanation/warnings only; manual picker remains available | PARTIAL / KNOWN LIMITATION — screenshot 47 proves normal explanation/candidates; provider failure, Network and audit persistence are open |
| 9 | Assign Developer | Smart Assign button | Bound `BugService.assignToDeveloper` | `assignToDeveloper` -> `transitionBug`; coordinator role and assignee validation | Updates assignee/status/next processor/history/notification | PARTIAL / KNOWN LIMITATION — Assigned create/reload and manual decision controls are proven; the bound assign request, invalid assignee, history and notification are not |
| 10 | Move to Pending Assignment | Fiori action dialog | Bound `moveToPendingAssignment` | `transitionBug`; role and transition checks | Clears assignee; writes dedicated audit and notification | KNOWN LIMITATION — dedicated action request/result/history was not executed |
| 11 | Mark In Review | Fiori action dialog | Bound `markInReview` | `transitionBug`; assigned Developer or coordinator, assignee required | Status In Review; dedicated audit/notification | KNOWN LIMITATION — request/result/history was not executed |
| 12 | Request More Information | Fiori action dialog | Bound `requestMoreInformation` | `transitionBug`; reason and assignee required | Status Need More Information; next processor changes | PARTIAL / KNOWN LIMITATION — control/dialog/required reason are captured; negative/positive request and persistence are open |
| 13 | Resubmit to Developer | Fiori action dialog | Bound `resubmitToDeveloper` | Dedicated handler; Tester/PM, note and assignee required | Status Assigned; adds comment, history and notification | KNOWN LIMITATION — negative/positive request and side-effect evidence was not executed |
| 14 | Reject Bug | Fiori action dialog | Bound `rejectBug` | `transitionBug`; assigned Developer/coordinator; reason required | Status Rejected; reason and follow-up owner persisted | PARTIAL / KNOWN LIMITATION — dialog/required reason are captured; negative/positive request and persistence are open |
| 15 | Start Progress | Fiori action dialog | Bound `startProgress` | `transitionBug`; assignee and transition checks | Status In Progress; dedicated audit/notification | KNOWN LIMITATION — assigned UAT execution is separate under IDTS-111; this package has no request/result/history evidence |
| 16 | Resolve Bug | Fiori action dialog | Bound `resolveBug` | `transitionBug`; note and assignee required | Status Resolved; Tester/PM follow-up | PARTIAL / KNOWN LIMITATION — dialog/required note are captured; negative/positive request and persistence are open |
...
| 21 | Queue attachment on create | `onAttachmentSelected` | Client memory only before activation | Client checks plus attachment metadata contract | No database/S3 write until active Bug exists | KNOWN LIMITATION — pre-activation pending-list/no-upload-request evidence was not captured |
| 22 | Upload attachment | Pending flush or active selection | Attachment metadata plus binary stream request | `prepareAttachmentWrite`; attachment plugin validation | Persists metadata and binary; row appears after refresh | PARTIAL / DEFERRED OWNER ACTION — safe selection and failed upload UI exist; successful Network, HANA metadata and S3 binary evidence remain DonHV/runtime follow-up |
| 23 | Download attachment | `onDownloadAttachment` | Attachment content stream GET | Attachment authorization/plugin | Returns binary with safe filename; no write | KNOWN LIMITATION — download/hash evidence was not available because upload did not persist |
| 24 | Delete attachment | `onDeleteAttachment` after confirm | Attachment DELETE | Attachment authorization/plugin | Deletes metadata/object without deleting Bug | KNOWN LIMITATION — before/request/after/reload evidence was unavailable because no controlled attachment persisted |
| 25 | Read history | History fragment growing list | Read `HistoryEvents` and nested `HistoryLogs` | `history-read-models.js` enriches paged read model | Returns immutable grouped events; Show More requests next page | PARTIAL / KNOWN LIMITATION — existing events/detail controls are captured; next-page request and expanded nested changes are open |
...
| 28 | Read notifications | Object Page Notifications facet | OData navigation read | Read-only service projection | Returns safe in-app/delivery status fields | PARTIAL / DEFERRED OWNER ACTION — in-app rows are captured; refresh, delivery/outbox comparison and provider readback remain DonHV/runtime follow-up |
| 29 | Read role dashboard | `dashboard-page.js::loadDashboard` | Reads Bugs and DeveloperWorkloads | CAP read models and auth | Builds role-specific tiles/focus rows without mutation | UI PASS / NETWORK KNOWN LIMITATION — Tester, Developer and PM dashboards are captured; sanitized OData requests are not |
| 30 | Read AI activity | PM dashboard dialog | Function `readAiOperationalMetrics` | `metrics.js`; PM requirement and allowlisted aggregates | Returns counts/latency only; no raw provider payload | PARTIAL / KNOWN LIMITATION — screenshot 50 proves authorized PM success; non-PM denial and Network response are open |

### 3.1 Line-level source verification — refreshed 2026-08-03
...
| EVID-108-UI-ROLES | List/Object Page/dashboard for Tester, Developer and PM | SangVN | Use demo users; avoid full private email | UI PASS — Developer, Tester and PM List Report/dashboard/Object Page variants are captured through the SAP BTP/approuter flow. Screenshots 48–51 add the PM monitoring tabs, workload dashboard, PM-only AI Activity dialog and BUG-0024 coordinator view; no profile-menu screenshot was retained because it exposed a private email |
...
| `screenshots/48-pm-list-report.png` | Authenticated PM List Report with monitoring filters, six business tabs, PM Action Queue, Dashboard/Create actions and BUG-0024 visible | Read-only role evidence; no filter, Create, row action or workflow mutation was submitted |
| `screenshots/49-pm-dashboard.png` | PM dashboard with open, pending-assignment and overdue KPIs plus needs-attention rows | Point-in-time monitoring evidence; no navigation or mutation was submitted |
| `screenshots/50-pm-ai-activity.png` | PM-only AI Activity dialog with Similar Bugs, Classification, Handoff Summary and Smart Assign request/result/latency/review aggregates | Read-only authorized-PM evidence; no raw provider payload, prompt, endpoint, credential or private identifier is visible; non-PM denial remains open |
| `screenshots/51-pm-bug-0024-object-page.png` | BUG-0024 Object Page under the PM session with Assigned status, SangVN technical/current owner and PM coordinator actions | Read-only PM role variant; no Edit, Move to Pending Assignment, comment, upload or lifecycle action was submitted |
...
- [x] SangVN personally acknowledges briefing SHA `3e78b495cb8feb56188cc446b827d47e040e1b98` in Jira comments `10876` and `10877`, including assigned Developer-role UAT ownership. Repository-register integration remains with DonHV under IDTS-105.
...
1. SangVN briefing acknowledgment is `READ` for SHA `3e78b495` in Jira comments
   `10876` and `10877`. The repository register on the current baseline remains an
   IDTS-105 integration action; candidate approval remains separate and Pending for
   the exact refreshed PR head.
...
3. CAP MCP and UI5 MCP read-only verification are available and were rerun on the
   refreshed worktree. Fiori MCP cannot be called because its mandatory `tools/list`
   protocol is not exposed; IDTS-112 should rerun it when available. Local UI5
   manifest validation, lint and build remain required in this package.
4. Every still-unexecuted row is a documented `KNOWN LIMITATION` or `DEFERRED OWNER
   ACTION`; none is represented as PASS. DonHV must disposition these limitations
   before IDTS-112 final integration.
