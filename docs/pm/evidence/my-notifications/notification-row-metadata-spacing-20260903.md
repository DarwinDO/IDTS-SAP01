...
- Visual row: available automated DonHV and NhanT sessions initially contained zero personal notification rows, so no synthetic notification was created. DonHV then supplied a current live screenshot of an existing read Bug notification; `Bug`, `Assigned`, `Read`, and `Action required` are visibly separated. The original concatenation defect is accepted live.
...
- Visual row: hai session automation đã đăng nhập DonHV và NhanT ban đầu đều có zero personal notification row, nên không tạo notification giả. Sau đó DonHV cung cấp ảnh live hiện tại của một Bug notification đã đọc; `Bug`, `Assigned`, `Read`, `Action required` đã tách rõ. Lỗi dính chữ ban đầu được accept trên live.
