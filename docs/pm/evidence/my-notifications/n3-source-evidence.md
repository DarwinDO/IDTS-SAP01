...
Final whole-branch review at `b74b68d20e98abbe33cd2286a093178a402cd259` returned 0 Critical / 0 Major / 1 Important / 1 Minor. The Important missing `COMMENT_MENTIONED` shell allowlist entry was fixed at `979a3ef878b0cf58aa55796203845788852030b1`; scoped re-review returned GO with 0 Critical / 0 Major / 0 Important and no new finding. The safe Minor remains explicitly deferred: upper/lowercase UUID variants in mention API are rejected instead of normalized/deduplicated; normal UI emits canonical IDs and no authorization bypass exists. Fresh final matrix at `979a3ef878b0cf58aa55796203845788852030b1` repeats all source tests above plus CAP/HANA compile, UI lint/build and diff/protected guards PASS. The known attachment warning remains unchanged. No runtime/browser/provider delivery or native Security diff-scan PASS is claimed.

Exactly one N3 PR was created: [#364](https://github.com/DarwinDO/IDTS-SAP01/pull/364), OPEN/Draft targeting `dev`. Initial pushed head was `9a79568ffbce074139f9c308f1e4b4b37c351a1a`; this closure changes PM/evidence documents only. The remote head, body validation and GitHub QA check are read back after the final closure push. Stop remains before Ready, merge, deploy, live migration/backfill/email/data/provider or N4.
...
Final review toàn branch tại `b74b68d20e98abbe33cd2286a093178a402cd259` trả 0 Critical / 0 Major / 1 Important / 1 Minor. Important thiếu `COMMENT_MENTIONED` trong allowlist shell đã fix tại `979a3ef878b0cf58aa55796203845788852030b1`; scoped re-review GO với 0 Critical/Major/Important và không có finding mới. Minor an toàn được defer rõ: UUID hoa/thường ở API mention bị reject thay vì normalize/dedupe; UI chuẩn gửi ID canonical và không có bypass quyền. Full matrix cuối mới tại SHA trên lặp lại toàn bộ test source, compile CAP/HANA, UI lint/build, diff/protected guard đều PASS. Warning attachment có sẵn không đổi. Không ghi runtime/browser/provider delivery hoặc native Security diff-scan PASS.

Đã tạo đúng một PR N3 [#364](https://github.com/DarwinDO/IDTS-SAP01/pull/364), OPEN/Draft vào `dev`. Head push đầu `9a79568ffbce074139f9c308f1e4b4b37c351a1a`; closure này chỉ đổi PM/evidence docs. Sẽ đọc lại remote head, PR body và GitHub QA sau push closure cuối. Dừng trước Ready, merge, deploy, migration/backfill/email-data-provider thật hoặc N4.
