...
| Functional Specification (EN/VI XLSX) | Đã sync in-place; sẵn sàng review | v0.4 giữ toàn bộ sheet workbook gốc và map workflow, attachment, PM monitoring, AI advisory hiện tại. OfficeCLI validation và format/content scan đều sạch. | Review trực tiếp file Drive hiện có; chỉ regenerate sau khi source/artifact được duyệt thay đổi. |
| Technical Specification (EN/VI XLSX) | Đã sync in-place; sẵn sàng review | v0.2 map CAP/Fiori component, standard, screen behavior, AI boundary an toàn và message. Cover kỹ thuật, metadata template, narrative area và field table đã được sửa mà không đổi sheet set của template. OfficeCLI validation và format/content scan đều sạch. | Review trực tiếp file Drive hiện có; Markdown/source local vẫn là canonical và chỉ regenerate sau khi source/artifact được duyệt thay đổi. |
...
Drive chỉ là bản review/distribution, không phải source of truth. Thông thường chỉ upload raw Office file mới có timestamp vào thư mục review `SU26SAP01_GSU26SAP01` sau khi source/artifact được duyệt thay đổi; không delete file mentor-review cũ. Khi review owner yêu cầu giữ ổn định Drive ID/link, chỉ thay bytes raw Office in-place sau khi local validation và đọc metadata mới. Sau mỗi lần upload phải đọc lại metadata folder/file. Chỉ ghi URL/ID trả về trong kênh handover vận hành được duyệt, không ghi vào source có track Git.
