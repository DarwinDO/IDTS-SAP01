...
| Functional Specification (EN/VI XLSX) | Ready for Drive review | v0.3 preserves all original workbook sheets and maps current workflow, attachment, PM, and advisory-AI behavior. | Upload raw XLSX copies. |
| Technical Specification (EN/VI XLSX) | Ready for Drive review | Template-derived v0.1 maps CAP/Fiori components, standards, screen behavior, safe AI boundary, and messages. | Upload raw XLSX copies; retain local Markdown/source as canonical. |
...
Drive is a review/distribution copy, never the source of truth. Upload a new timestamped raw Office file to the `SU26SAP01_GSU26SAP01` review root only after an approved source/artifact change; do not overwrite or delete prior mentor-review files. Read the folder back after each upload. Record any returned URL/ID only in the approved operational handover channel, never in Git-tracked source.
