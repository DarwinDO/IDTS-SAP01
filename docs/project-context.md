...
- An active PM with `UserAdmin` may cancel any invitation only while it is unverified and exactly `INVITED`. Cancellation invalidates the old link, releases the open-request key, stops pending delivery, and preserves request/delivery/audit history so a corrected invitation can be sent safely.
- Vietnamese: PM active co `UserAdmin` chi duoc Cancel invitation khi request chua verify va dung trang thai `INVITED`. Cancel lam link cu mat hieu luc, giai phong open-request key, dung delivery dang cho va giu lich su request/delivery/audit de co the gui invitation sua dung mot cach an toan.
- The source-candidate administration flow continues through explicit approval and a versioned operation journal. UI never calls SAP authorization APIs. CAP queues only allowlisted operations; a separate broker performs read-before/write/read-after Role Collection reconciliation and returns sanitized semantic results. `ACTIVE` requires provider proof. Role change/revoke/suspend suspend local access and revoke `AuthSessions` before reconciliation; local suspend uses `SUSPENDED` and performs no provider write, while reactivation remains inactive until a read-only broker operation proves the immutable principal and exact desired Role Collection set. The real SAP adapter/credential and HANA/XSUAA deployment remain gated and are not live merely because local fake-broker tests pass.
...
- Legacy TESTER/DEVELOPER rows are linked through a one-time invitation that targets one explicit `Users.ID`. The broker performs read-only exact-role proof, then CAP updates the same row with the verified email and immutable identity tuple in one transaction. IDTS never guesses a match or creates a replacement row for this transition. Unlinked legacy Developers are excluded from new assignment/Smart Assign, while existing Bug assignees remain unchanged.
...
- Job Scheduler remains the recovery/retry path and adds scheduled Pending Assignment/Overdue discovery, four-hour Critical/Blocker or 24-hour normal SLA reminders, and weekday 08:00 `Asia/Bangkok` digest generation. Automated inbox-index cleanup is deferred until measured volume justifies a separately reviewed destructive operation; N5-Lite adds read-only Digest diagnostics only.
...
## Gate 5 Business Catalog Administration context

The IDTS User Administration workspace owns bounded maintenance of the four classification catalogs used by Bug classification and Developer Responsibilities. CAP is authoritative: exact PM + UserAdmin, active internal PM, normalized uniqueness, active-parent validation, ETag conflict protection, impact/deactivation guards, no DELETE, and sanitized append-only audit. The UI never exposes HANA, provider, credential, or generic schema controls.

Workspace User Administration IDTS quản lý giới hạn bốn classification catalog dùng cho Bug classification và Developer Responsibility. CAP là authority: đúng PM + UserAdmin, internal PM active, uniqueness đã normalize, validate parent active, ETag conflict, impact/deactivation guard, cấm DELETE và audit append-only sanitize. UI không expose HANA, provider, credential hoặc generic schema control.

## Gate 6 Operations and Audit context

**English:** Gate 6 adds safe, bounded PM + UserAdmin operational visibility without exposing the persistence entities directly. `UserAdministrationService` returns explicit onboarding-delivery, access-operation, audit-event, and readiness DTOs; every query uses stable ordering and server paging of 25 by default/100 maximum. The UI keeps Delivery and Provisioning under Operations and Audit separate, lazy-loads models, masks display values, and offers only existing state-valid access actions plus one optimistic, capped onboarding-delivery retry for an unexpired `INVITED` parent request. Readiness comes only from persisted safe outcome timestamps within a fixed seven-day freshness window: delivery `sentAt`/`lastAttemptAt` and operation `completedAt`. No schema/index change, provider outage, endpoint/binding/credential/log control, raw email/provider data, lease/lock/idempotency value, identity hash, token, JWT, or raw error is part of this gate.

**Tiếng Việt:** Gate 6 thêm operational visibility an toàn, có giới hạn cho PM + UserAdmin mà không expose trực tiếp persistence entity. `UserAdministrationService` trả DTO explicit cho onboarding delivery, access operation, audit event và readiness; query dùng order ổn định và paging server mặc định 25/tối đa 100. UI tách Delivery/Provisioning trong Operations và Audit riêng, lazy-load model, mask display value và chỉ có access action đúng state hiện có cùng một retry onboarding delivery optimistic có ceiling cho parent invitation còn `INVITED` và chưa hết hạn. Readiness chỉ lấy từ timestamp outcome an toàn đã persist trong freshness window cố định bảy ngày: delivery `sentAt`/`lastAttemptAt` và operation `completedAt`. Gate này không có schema/index change, outage provider, control endpoint/binding/credential/log, raw email/provider data, lease/lock/idempotency, identity hash, token, JWT hoặc raw error.
